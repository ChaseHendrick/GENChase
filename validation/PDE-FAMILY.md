...
Supporting cross-checks, 2026-09-25 (`npm run xref`, [tools/xref/](../tools/xref/README.md)), which do not change any status. Each exported default field (grid 256, noise off, SwiftShader float32) is compared with an independent ETDRK4 pseudo-spectral solution of the published equation, started from the same initial field. The comparison is made against a reference that uses the 5-point lattice symbol and against a continuum reference.

- **Swift–Hohenberg** (2000 steps, T = 30.29). Against the lattice reference, the relative L2 difference is 1.35e-3; the computed first-order time bound is 5.63e-2. Against the continuum reference it is 1.5e-1, which is reported rather than gated. The dominant wavenumber is 0.5554 in the studio and 0.5488 in the continuum reference, 1.2 per cent apart (tolerance 5 per cent). PASS. The default g = 0 leaves the quadratic term unexercised.
- **Kuramoto–Sivashinsky.** The pointwise comparison stops at 150 steps (T = 2.7), before chaotic separation. There the relative L2 difference against the lattice reference is 1.52e-3, with a time bound of 3.14e-3. The long run lasts 2000 steps (T = 36). Over the unstable band k ≤ 1, the energy-spectrum shape differs from the continuum reference by 0.071 in RMS log10 ratio (tolerance 0.15). PASS. In the dissipative tail 1 < k ≤ 2, the shape differs by 0.222 from the continuum reference and by 0.012 from the lattice reference. That tail difference is the fixed-spacing stencil's weaker damping, not a code error.
