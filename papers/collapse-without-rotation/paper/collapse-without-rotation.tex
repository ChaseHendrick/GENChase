...
For $n = 2$ and $c \le 1/4$, and for every even $n = 2m \ge 4$ and $c \le \min(1/2, 2\sin(\pi/m))^2$, the constant $\sqrt{3}/2$ in \textup{(a)} cannot be increased; for $n = 3$ and $c \le 1/2$ see Corollary~\ref{cor:triple}.
\end{theorem}
...
For $n = 1$, part (d) gives $P \to \infty$, so (a) holds with any constant. For $n = 3$ the constant $\sqrt{3}/2$ is sharp for $c \le 1/2$, by Corollary~\ref{cor:triple}. For odd $n \ge 5$ we have only numerical evidence: a strong vortex with a pair and a triple ($n = 5$) has least $P$ tending to $\sqrt{3}/2$ from above (Table~\ref{tab:clusters}).
...
The slope in \eqref{eq:triple} is the one \eqref{eq:formal} gives for an equilateral triple, $-(2/\sqrt{3})\,e_C/\gamma$ with $e_C = -3\gamma g_1g_2g_3/S$. By Theorem~\ref{thm:clusters}(b) every collapse with bounded $P$ has $P$ close to $(y_C^2 + 3/4)/(2y_C)$; for a triple, Theorem~\ref{thm:triple} shows that every value on that curve is attained in the limit $\gamma \to 0$.
...
\begin{remark}[Larger clusters]\label{rem:triplegeneral}
The proof uses only (I1), (I2) and the nondegeneracy of the triangle. It applies without change to one cluster of $m \ge 2$ vortices that, at $\gamma = 0$, is a translating relative equilibrium with circulations of zero sum at which the derivatives of $w_k - w_1$, $k \ge 2$, have the scaling direction as their only common kernel direction, and gives an analytic family with $P(0) = (y_0^2 + 3/4)/(2y_0)$. For $m = 2$ the condition holds trivially; the cluster is a pair, its first-order coefficient vanishes ($e_C = 0$), and \cite[Proposition~4]{mw2026} bounds the next order from below. We have computed the first-order coefficient exactly only for triples; for larger clusters, and for several clusters, \eqref{eq:formal} remains formal.
\end{remark}
...
\caption{The verification programs, what they check, and the number of checks.}\label{tab:checks}\\
  ...
  \texttt{verify\_\allowbreak triple\_\allowbreak branch.py} & Theorem~\ref{thm:triple} and Corollary~\ref{cor:triple}: the point at $\gamma = 0$, the nondegeneracy of the triangle and the slope $2\sqrt{3}\,g_1g_2g_3/S$, exactly over $\Q(t)(\omega)$; the family at forty digits for three triples and both $\omega$, checked against the Biot--Savart equations of all four vortices, with the convergence of the slope; other $y_0$; the class $\K(3, 1/2)$; controls. About ten seconds & $50$ \\ \hline
...
\item Does the continuum problem \eqref{eq:sheet} have a solution, and is the local minimum $P_\infty$ of its winding a known constant? What does the unread work of O'Neil \cite{oneil2010} say about it? Does a sheet with another behavior at its tips, or with more point vortices, wind less?
...
\item Can the expansion \eqref{eq:formal} be proved for several clusters, or for one cluster of more than three vortices (Remark~\ref{rem:triplegeneral}), and does a strong vortex with clusters whose $e_C$ are all nonpositive satisfy $P > \sqrt{3}/2$ at every small fixed $\gamma$, as pairs do \cite[Proposition~4]{mw2026}?
\end{enumerate}
