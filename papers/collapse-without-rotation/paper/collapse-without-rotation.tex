...
The proof uses only (I1), (I2) and the nondegeneracy of the triangle. It applies without change to one cluster of $m \ge 2$ vortices with circulations $g_k$ of zero sum, the last of them, $g_m \ne 0$, carrying the correction $\gamma\nu$, that at $\gamma = 0$ is a translating relative equilibrium with $w_1 = \cdots = w_m \ne 0$ at which the derivatives of $w_k - w_1$, $k = 2, \ldots, m - 1$, taken on the subspace $\sum_k \zeta_k = 0$ (parametrized by $\zeta_1, \ldots, \zeta_{m-1}$), have the scaling direction as their only common kernel direction. It gives an analytic family with $P(0) = (y_0^2 + 3/4)/(2y_0)$. For $m = 2$ the condition holds trivially and the cluster is a pair; at $y_0 = \sqrt{3}/2$, \cite[Proposition~4]{mw2026}, applied to both signs of $\gamma$ (a negative $\gamma$ exchanges the signs of the pair), gives $P \ge \sqrt{3}/2$ on both sides of $\gamma = 0$, so the first-order coefficient vanishes, as $e_C = 0$ in \eqref{eq:formal} says. We have computed the first-order coefficient exactly only for triples; for larger clusters, and for several clusters, \eqref{eq:formal} remains formal.
\end{remark}
