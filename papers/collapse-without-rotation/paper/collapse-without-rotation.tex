...
For $n = 2$ and $c \le 1/4$, and for every even $n = 2m \ge 4$ and $c \le \min(1/2, 2\sin(\pi/m))^2$, the constant $\sqrt{3}/2$ in \textup{(a)} cannot be increased.
\end{theorem}
...
For pairs, $P > \sqrt{3}/2$ at every $\gamma$ below a threshold \cite[Proposition~4]{mw2026}. For clusters we expect no such statement: collapses of a strong vortex with a single triple of signs $(+,+,-)$, computed at fifty digits, have $P < \sqrt{3}/2$ at $\gamma = 10^{-2}$, $10^{-3}$ and $10^{-4}$ (Section~\ref{sec:formal} and Table~\ref{tab:clusters}; numerical), and the formal expansion \eqref{eq:formal} predicts this at every small $\gamma$. So, numerically and formally but not by proof, the $\delta$ in (a) cannot be removed: $\sqrt{3}/2$ is the limit of the least $P$ as $\gamma \to 0$, not a lower bound at a fixed $\gamma$.
\end{remark}
...
\bibitem{mw2026} C. Hendrick, Minimal winding in the self-similar collapse of point vortices, preprint (2026), doi:10.5281/zenodo.22953034; programs and data at \url{https://github.com/ChaseHendrick/minimal-winding}.
