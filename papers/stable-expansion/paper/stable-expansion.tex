...
\[
  \bigl|\zeta(s) - e^{i(\phi_\infty - (b(\sigma_\infty) - b^*)s)}\xi(\sigma_\infty)\bigr| \le C_1(\delta + \varepsilon)e^{-s}\qquad(s \ge 0).
\]
\end{proposition}
...
For the sharp rate, let $\mu > 1$ and $S = \infty$; by what was shown with $\nu = 1/2$ (after decreasing $\delta_1$ if necessary), $|\sigma(s) - \sigma_\infty| + |y(s)| \le C(\delta + \varepsilon)e^{-s/2}$. Write $y' = B(\sigma_\infty)y + r(s) + F(s)$, where $r = (B(\sigma(s)) - B(\sigma_\infty))y + (g(\sigma, y) - B(\sigma)y)$ satisfies $|r(s)| \le C(\delta + \varepsilon)e^{-s/2}|y(s)|$ and $F$, the forcing term of \eqref{eq:coords}, satisfies $|F(s)| \le C_L\varepsilon e^{-\mu s}$. By Lemma~\ref{lem:coords} and the variation of constants formula,
\[
  e^{s}|y(s)| \le K_B|y(0)| + \frac{K_BC_L\varepsilon}{\mu - 1} + K_BC(\delta + \varepsilon)\int_0^se^{-\tau/2}e^{\tau}|y(\tau)|\,d\tau,
\]
and Gronwall's inequality gives $|y(s)| \le C(\delta + \varepsilon)e^{-s}$. Then $|\sigma(s) - \sigma_\infty| \le \int_s^\infty(C_f|y| + C_L\varepsilon e^{-\mu\tau})\,d\tau \le C(\delta + \varepsilon)e^{-s}$. With $\beta_\infty = b(\sigma_\infty) - b^*$, $\phi' = h(\sigma, y) + O(\varepsilon e^{-\mu s}) = -\beta_\infty + O((\delta + \varepsilon)e^{-s})$, so $\phi(s) + \beta_\infty s$ converges to some $\phi_\infty$ at the rate $(\delta + \varepsilon)e^{-s}$, and the estimate follows from $\zeta(s) = e^{i\phi(s)}(\xi(\sigma(s)) + Qy(s))$.
\end{proof}
...
\begin{proof}
We follow the proof of \cite[Theorem~2]{zbarsky2021}, with the equation numbers of arXiv:1912.10862v2, and indicate every change. Zbarsky measures time in units of $2\pi$, so that $K(x) = x^\perp/|x|^2$; in his units the reference configuration is $\sqrt{2\pi}\,Y$ and the slice $\sqrt{2\pi}\,S_Y$, and (i) and (ii) in his units imply them in ours, since $t/2\pi < t$ and the ratios $\gamma/\sqrt t$ and $|z - Y|/|Y|$ do not change. We run his argument in his units, except in step (b), with his constants $k$ (a large even integer), $\delta_Z$ (small) and $R$ (large). His bootstrap assumptions, for $j = 1, \ldots, N$ and in his units, are:
(B1) $\bar x = e^{i\beta}\gamma z$ with $z \in \sqrt{2\pi}\,S_Y$, $|z - \sqrt{2\pi}\,Y| < \varepsilon^2$ and $|\gamma/\sqrt t - 1| < \varepsilon$;
(B2) $I_{2,j} < t^{\varepsilon/2}$;
(B3) $I_{k,j} < t^{k(1 + \varepsilon)/4}$;
...
Here $I_{k,j} = \int|x - \bar x_j|^k|\omega_j(x)|\,dx$. They hold at $t_0$ when $T$ is large; for (B1) this follows from \eqref{eq:slicebound}, since $|\bar x(t_0) - \sqrt{t_0}\,Y| \le \sqrt N\,r$. Suppose that one of them fails at a first time $T_* < \infty$. They hold on $[t_0, T_*)$, and at $T_*$ with $\le$ in place of $<$.

\emph{Where $N = 3$ entered.} Zbarsky's proof uses three vortices in one place: to recover (B1), at the end of his Section~3. There he uses that the angular impulse and the energy of the centres are nearly conserved, and that for three vortices these two quantities, with the rotation and the dilation, are coordinates near the reference configuration \cite[Lemma~1(ii)]{zbarsky2021}; the dilation then follows from $d|\bar x_1|^2/dt$. For $N \ge 4$ the configurations modulo rotation and dilation have dimension $2N - 4 > 2$, and two nearly conserved quantities cannot pin them down. We use Proposition~\ref{prop:forced} instead. Two further facts enter his proof, and both hold here: $\sum_{i<j}\Gamma_i\Gamma_j = 0$, exactly for our circulations (the program checks it in rational arithmetic; it is necessary for any self-similar expansion, since $H(\lambda\zeta) = H(\zeta) - \ln\lambda\sum_{i<j}\Gamma_i\Gamma_j$), and $\sum_j\Gamma_j \ne 0$, which follows because $(\sum_j\Gamma_j)^2 = \sum_j\Gamma_j^2 + 2\sum_{i<j}\Gamma_i\Gamma_j > 0$ (the sums are $253/90$ and $93/40$).

\emph{(a) The centres move as point vortices up to a small error.} By (B1), $c\sqrt t \le |\bar x_i - \bar x_j| \le C\sqrt t$ and $|\bar x_j| \le C\sqrt t$ for $i \ne j$, with $c$, $C$ depending only on $Y$, and by (B5) the patches lie in disjoint balls of radius $o(\sqrt t)$ about their centres. Zbarsky's computation on his pp.~7--8, with the other patches summed over $j \ne i$, expands the kernel between two patches about their centres of vorticity. The first-order terms vanish because $\bar x_j$ are the centres, the self-interaction vanishes by antisymmetry, and the remainder is $O((I_{2,i} + I_{2,j})t^{-3/2})$, which is $O(t^{\varepsilon/2 - 3/2})$ by (B2). Hence, in the units of \eqref{eq:bs},
\[
  \frac{d\bar x}{dt} = V_{-\Gamma}(\bar x) + e(t),\qquad |e(t)| \le Kt^{\varepsilon/2 - 3/2}\quad(t_0 \le t \le T_*),
\]
with $V_{-\Gamma}$ the velocity \eqref{eq:bs} for the circulations $-\Gamma$ and $K$ depending on $Y$, $\varepsilon$, $M$ and $r$ but not on $T_*$. Also $|d\bar x_j/dt| = O(t^{-1/2})$.

\emph{(b) Recovering (B1).} Put $\zeta(s) = \rho^{-1}e^{ib^*s}\bar x(t)$ with $\rho = \sqrt{t/\pi}$ and $s = \ln\rho$. The computation after Proposition~\ref{prop:forced} uses only $d\rho/dt = 1/(2\pi\rho)$, so $\zeta' = -E(\zeta) + 2\pi\rho e^{ib^*s}e(t)$, and $2\pi\rho|e(t)| \le 2\pi K\rho(\pi\rho^2)^{\varepsilon/2 - 3/2} \le 2\pi K e^{-(2 - \varepsilon)s}$. In the shifted time $s - s_0$, $s_0 = \ln\rho(t_0)$, the hypothesis of Proposition~\ref{prop:forced} holds with $\mu = 2 - \varepsilon$, forcing $\varepsilon_1 = 2\pi K\rho(t_0)^{\varepsilon - 2}$, and $\delta = \operatorname{dist}(\zeta(s_0), \mathcal O) \le |\bar x(t_0) - \sqrt{t_0}\,Y|/\rho(t_0) \le \sqrt{\pi N}\,r\,t_0^{-1/2}$. Both tend to $0$ as $t_0 \to \infty$, so for $T$ large the proposition, applied with $\nu = 1/2$ on $[s_0, \ln\rho(T_*))$ and extended to $T_*$ by continuity, gives $\operatorname{dist}(\zeta(s), \mathcal O) \le C_1(\delta + \varepsilon_1)$ on $[t_0, T_*]$. Since $\min_\phi|\bar x(t) - \sqrt t\,e^{i\phi}Y| = \sqrt{t/\pi}\,\operatorname{dist}(\zeta(s), \mathcal O)$, and $\bar x(t) \in \mathcal V$ because $\sum_j\Gamma_j\bar x_j = -\sum_j\int x\,\omega_j$ is conserved and vanishes at $t_0$, \eqref{eq:slicebound} with $\lambda = \sqrt t$ gives $|z - Y| \le 4C_1(\delta + \varepsilon_1)/\sqrt\pi$ and $|\gamma/\sqrt t - 1| \le C_1(\delta + \varepsilon_1)/(\sqrt\pi|Y|)$ on $[t_0, T_*]$. In his units $|z - Y|$ is multiplied by $\sqrt{2\pi}$, and for $T$ large both bounds are less than $\varepsilon^2/2$. So (B1) holds on $[t_0, T_*]$ with strict inequalities, in particular at $T_*$.

\emph{(c) The other assumptions.} Zbarsky's Sections~4, 4.1 and~5 treat one patch at a time and use (B1) only through the bounds in (a). For $N$ patches, $v_2 + v_3$ becomes $\sum_{j\ne1}v_j$ in (3)--(5), (19), (34) and (35). The renormalizing functions $f_{k,2}$, $f_{k,3}$ in (11)--(18) become $f_{k,j}$, one for each $j \ne 1$, and the angles $\theta_2$, $\theta_3$ in (29)--(31) become $\theta_j$, $j \ne 1$. For each $j$ separately, the principal term in (30) averages to zero over one turn of $p$ about $\bar x_1$. These sums have $N - 1$ terms, which changes only constants. The concentration estimate (33) uses the conserved energy $\iint\log|x - x'|\,\omega(x)\omega(x')\,dx\,dx'$; there the cross terms contribute $\sum_{i\ne j}\Gamma_i\Gamma_j(\ln t)/2 + O(1) = O(1)$, because $\sum_{i<j}\Gamma_i\Gamma_j = 0$. One term is missing from (34) in the arXiv version, for any $N$: $f_{k,j}$ depends on $t$ through $\bar x_1$ and $\bar x_j$, and the omitted $\int\partial_tf_{k,j}\,\omega_1$ is $O(t^{-1/4+\varepsilon}I_{k,1})$ by (B5), $|d\bar x_1/dt| = O(t^{-1/2})$ and (29), the same order as the term $O(t^{-1/2}\sup|x - \bar x_1|\,I_{k,1})$ already in (35). It is absorbed in the error $O(\delta_ZI_{k,1})$ of (12) once $T$ is large, $T$ being chosen after $\delta_Z$. With these changes his arguments recover (B2), (B3) and (B5) at $T_*$, in both the case $T_* \ge t_0 + t_0^{9/10}$ (his Section~4) and the case $T_* < t_0 + t_0^{9/10}$ (his Section~5). (B4) holds because the vorticity is transported by a divergence-free field. This contradicts the choice of $T_*$, so every assumption holds for all $t \ge t_0$, which gives (i) and (ii).

\emph{(d) Part (iii).} Now the estimate of (a) holds for all $t \ge t_0$, so Proposition~\ref{prop:forced} applies on $[s_0, \infty)$ with $\mu = 2 - \varepsilon > 1$, and its last part gives $\sigma_\infty$ with $|\sigma_\infty| \le C_1(\delta + \varepsilon_1) < \varepsilon$ and $\phi_1$ with $|\zeta(s) - e^{i(\phi_1 - (b(\sigma_\infty) - b^*)(s - s_0))}\xi(\sigma_\infty)| \le C_1(\delta + \varepsilon_1)e^{-(s - s_0)}$. Multiplying by $\rho e^{-ib^*s}$, and using $\rho e^{-(s - s_0)} = \rho(t_0)$, gives $|\bar x(t) - Z(t)| \le C_1(\delta + \varepsilon_1)\rho(t_0) \le C_1(\sqrt N\,r + \varepsilon_1\rho(t_0))$, which is bounded, with $\phi_\infty = \phi_1 + (b(\sigma_\infty) - b^*)s_0$. $Z$ is a motion by the proof of Corollary~\ref{cor:physical}.
\end{proof}
