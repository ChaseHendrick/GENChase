...
*Theorem 1.* _Let $alpha > -1$. Every self-similar collapse of three point vortices under @eq:bs has $P > B(alpha)$, and for every $delta > 0$ there is one with $P < B(alpha) + delta$._

The proof (Section 4) needs two facts proved in Section 3: a collapsing triangle is scalene and carries circulations fixed up to a factor by its side lengths (Lemma 2), and $P$ is a symmetric function of the side lengths alone (Lemma 3). For $alpha = 0$ it gives a short proof of the Euler bound of [8], though not the minimum for each ratio of circulations obtained there. Section 5 records that the bound does not extend to more than three vortices in the Euler case, and Section 6 lists the numerical checks.
...
The spiral form @eq:spiral of a self-similar collapse in the $alpha$-models is in [15] and [9, Eqs. (3)–(6)], in [5, (A.19)], whose exponent $1\/(alpha + 1)$, in a parametrization where the Euler case is $alpha = 1$, is our $1\/(2 beta)$, and in [7, Prop. 2.1]. For $beta = 1$, $P = |omega_0| t_c$ is the product studied in [8]. For $beta != 1$ the product $|omega_0| t_c$ is the rotation per unit decrease of $ln (t_c - t)$, and it differs from $P$ by the factor $1\/beta$; we state the bound for $P$, which measures the geometry of the spiral, and for $|omega_0| t_c$ in Corollary 1.
...
Throughout, $alpha > -1$, that is, $beta > 1\/2$. We use the elementary inequality
...
holds for all $rho$, $m$, $beta$, and its right side is positive. If $m < 2$, the second bracket exceeds $1 + 6 beta - 4 beta - 2 = 2 beta - 1 > 0$. If $m >= 2$, then $2\/m <= 1$ and $rho < 1$, so $(1 + beta) m - 2\/m - rho > (1 + beta) m - 2 > 0$, and the second bracket is at least $b(m) = 1 + 6 beta - beta m^2 - m$. If $b(m) >= 0$ the right side is positive; otherwise $rho^2 b(m) > b(m)$, and the right side exceeds $D(m) = ((1 + beta) m - 2)^2 + b(m)\/3$. Now $D'' = 2 beta^2 + 10 beta \/ 3 + 2 > 0$ and $D'(2) = 4 beta^2 + 8 beta\/3 - 1\/3 > 0$, so $D$ increases on $[2, infinity)$, and $D(2) = 4 beta^2 + (2 beta - 1)\/3 > 0$. Hence $Lambda > sqrt(1 + 2 beta) sqrt(4 - (m - rho)^2)$ and $P > sqrt(1 + 2 beta)\/(2 beta) = B(alpha)$.
...
In the extremal limit $rho -> 0$ the circulations @eq:circ are, up to a factor, $Gamma_2 approx 1$ at the vertex opposite the shortest side and $Gamma_1 approx -Gamma_3$ of order $rho^(2 beta - 1)$ at its ends: two weak vortices of opposite sign, at distance $rho$, spiral into a strong vortex, and the line through them makes the angle $arccos(1\/sqrt(2(1 + beta)))$ with the line to the strong vortex. For $alpha = 0$ this is the limit $mu -> 0$ of [8, Corollary 1].

*Corollary 1.* _Let $alpha > -1$. In every self-similar collapse of three point vortices under @eq:bs, each vortex travels more than $(4 + alpha)\/(2 + alpha)$ times its initial distance from the collision point, the angle between its path and the direction to that point exceeds $arccos((2 + alpha)\/(4 + alpha))$, and $|omega_0| t_c > 2 sqrt(3 + alpha)\/(2 + alpha)^2$. None of these constants can be improved._
...
Since $d ln B \/ d alpha = -(4 + alpha)\/(2 (3 + alpha)(2 + alpha)) < 0$, the bound decreases strictly in $alpha$, from $sqrt(3)\/2$ in the Euler case through $2\/3$ for SQG to $0$ as $alpha -> infinity$: more local interactions allow tighter spirals, but never a collapse without rotation (@fig:alpha). The restriction $alpha > -1$, that is, $2 beta > 1$, is the range in which the velocity @eq:bs induced by a vortex decays with distance; the proof uses it in step (iii).

*Remark 1 (below $alpha = -1$).* The sharpness argument holds for every $alpha > -2$, since exactly $S - rho m - rho^2 coth b = 2X((1 + rho m)\/(1 - X) - 1\/(Y - X))$, which is $O(X rho)$ for every $beta > 0$. Steps (i) and (ii) hold for every $beta > 0$, and so does the case $m >= 2$ of step (iii) as soon as $D(2) = (12 beta^2 + 2 beta - 1)\/3 > 0$. In the case $m < 2$, if $b(m) = 1 + 6 beta - beta m^2 - m > 0$ the right side of @eq:id is positive; otherwise, when $(1 + beta) m - 2\/m - 1 >= 0$, it exceeds $((1 + beta) m - 2\/m - 1)^2 + b(m)\/3$. An interval-arithmetic subdivision of $0.2622 <= beta <= 1\/2$, $3\/2 <= m <= 2$ (the program `verify_alpha_extension.py`) shows that one of these holds on every box, and $b(m) >= b(3\/2) = 15 beta\/4 - 1\/2 > 0$ for $m <= 3\/2$. Hence Theorem 1 and Corollary 1 hold for every $alpha >= -59\/40$, with this computer-assisted step. At $alpha = -1.5$ and $alpha = -1.8$ numerical minimization of @eq:S, in arithmetic whose precision grows with $|ln rho|$, finds no value below $sqrt(1 + 2 beta)\/(2 beta)$ either, but we do not claim a bound there.

*Remark 2 (two equal circulations).* For circulations $(1, -gamma, 1)$ the collapsing triangles form a single family. For $alpha = 2$ it is $r_1 r_3 = r_2^2$, where $r_2$ joins the two equal vortices, the ratio is $gamma = r_2^2\/(r_1^2 + r_3^2)$, and Lemma 3 gives
$ P^2 = ((1 + gamma)(2 gamma^2 - 2 gamma + 1)^2) / (4 (1 - 2 gamma)(1 + 2 gamma)(3 gamma - 1)) , quad 1/3 < gamma < 1/2 , $
whose minimum, $1.1039451679 dots$, is a root of $27648 y^8 - 29952 y^6 - 3200 y^4 - 1556 y^2 - 125$; at $gamma = 1\/3$ the triangle degenerates to the collinear configuration with sides in the ratio $phi : 1 : 1\/phi$, $phi$ the golden ratio. For SQG the interval $0.387464 dots < gamma < 1\/2$ of [1, 2] has as its lower end the root in $(0, 1\/2)$ of $4 gamma^4 - 8 gamma^3 + gamma^2 - 2 gamma + 1$, where the triangle becomes collinear.
...
Families of larger numerical minimizers go lower still: $P = 0.5172291322 dots$ for 33 vortices and $P = 0.4981844392 dots$ for 61 vortices, so a self-similar collapse can turn by less than half a radian per unit decrease of $ln lambda^2$. Whether $P$ can be arbitrarily small for large numbers of vortices is open; the families computed so far extrapolate to about $0.477$. The same happens for $alpha != 0$: minimizing over four vortices in the same way gives $P_4 = 0.5499151189 dots$ for SQG, against $B(1) = 2\/3$, and $P_4 = 0.4116738600 dots$ for $alpha = 2$, against $B(2) = sqrt(5)\/4$, both strict local minima with one strong and three weak vortices, as for $alpha = 0$. These are numerical results, not proofs.

= Numerical verification

The program `verify_alpha_winding.py` checks each step in high-precision arithmetic, and its output is `data/verify_alpha_winding.txt`. On 700 random scalene triangles with $alpha in {0, 0.3, 1, 1.7, 3}$, the circulations @eq:circ make the Biot–Savart velocities @eq:bs self-similar to $10^(-38)$, and $P$ computed from the velocities agrees with @eq:S to $3 dot 10^(-38)$. The derivative of $g$, the identity @eq:id and the factorization of $h(rho) - h(1)$ are verified symbolically, and the critical point and value of $P_0$ as well. The chain of inequalities in the proof holds at $20000$ random points with $beta$ from just above $1\/2$ to $20$ and $rho$ down to $10^(-8)$, computed with precision adapted to $rho^(2 beta)$; in double precision the first step appears to fail at small $rho$ and large $beta$, where its margin, of order $rho^(2 beta)$, is below the rounding error. For $alpha$ from $-1.8$ to $-1$, outside the theorem, the minimum of $P - B(alpha)$ over the ordered triangles is positive in every run, again only with precision adapted to $rho$. For $alpha = 1$, the scalene triangles on which @eq:circ gives two equal circulations carry the ratios $-Gamma_2\/Gamma_1$ from $0.3876$ to $0.49999$ on a grid, in agreement with the interval of [1, Lemma 1], and at $Gamma = 0.49$ the ratio of sides is $0.7515$, against $0.751484$ in [1]. The program `verify_alpha_extension.py` performs the interval-arithmetic step of Remark 1, and `symmetric_family.py` in `research/generalizations-2026-09-24/checks/` checks Remark 2. Near-extremal triangles at $rho = 10^(-6)$ and $m = m_*$ collapse, and $P - B(alpha)$ is $1.3 dot 10^(-12)$, $1.8 dot 10^(-13)$, $1.6 dot 10^(-13)$ and $1.6 dot 10^(-13)$ for $alpha = 0, 1\/2, 1, 2$. The four-vortex minima for $alpha = 1, 2$ were refined by Newton's method on the Lagrange conditions in 80-digit arithmetic (residual $10^(-56)$), and 800 constrained perturbations of each lower $P$ in no case. The four-, thirty-three- and sixty-one-vortex configurations of Section 5 satisfy the self-similarity equations to $10^(-29)$, $10^(-39)$ and $10^(-28)$ relative in independent Biot–Savart checks.

= Open problems

+ Is there a positive lower bound for $P$ over self-similar collapses of any number of vortices in the Euler case? The smallest values found decrease with the number of vortices, to $0.4981844392 dots$ for 61 vortices, and the families computed extrapolate to about $0.477$.
+ Is $P_4 = 0.7978967838 dots$ of Section 5 the minimum over all collapses of four vortices, and is it algebraic? No integer polynomial of degree at most 16 with coefficients below $10^7$ vanishes at $P_4$ or $P_4^2$.
+ Does Theorem 1 hold for $-2 < alpha < -59\/40$? Numerically it does.
+ Are the four-vortex values $0.5499151189 dots$ (SQG) and $0.4116738600 dots$ ($alpha = 2$) of Section 5 the infima for four vortices, and what are the infima for more vortices when $alpha != 0$?
+ For fixed circulations and $alpha != 0$ only finitely many triangles collapse; for $alpha = 2$ they are at most two and $P$ is an explicit algebraic function of the circulations. Is there such a formula for SQG, where the collapsing triangles are the roots of a quartic?

= Discussion

Self-similar collapse of three vortices in the $alpha$-models has been studied from several directions: Badin and Barry [1] derive the necessary conditions for collapse in Nambu form and show that in the SQG case collapse is self-similar when the energy vanishes and not self-similar otherwise; for circulations $(1, -Gamma, 1)$ they find self-similar SQG collapse exactly for $0.387464 dots < Gamma < 1\/2$ [1, Lemma 1], an interval that Lemma 2 reproduces; Reinaud, Dritschel and Scott [14] give the conditions for self-similar collapse of three vortices and map the collapse time over the collapsing configurations; Reinaud [13] gives, for SQG, two necessary conditions and the collapse time in terms of the side lengths (as quoted in [2]), which has the form of the rate $op("Re") kappa$ of Lemma 2 at $alpha = 1$; Chen and Liu [2] prove that the existence of self-similar collapse and expansion depends only on the circulations, give explicit expressions for all nontrivial self-similar solutions [2, Theorems 2.1, 2.2], and obtain the interval of Badin and Barry analytically [2, Theorem 4.1], a description from the side of the circulations that Lemma 2 complements from the side of the shape; Yasunaga, Otobe and Iwayama [15] give necessary conditions and the orbits of self-similar motion, and Iwayama and Yajima [9] show that the collapsing motions are linearly unstable and the expanding ones stable; Iwayama, Yajima and Watanabe [10] prove that collapse that is not self-similar can occur for $0 < alpha < 2$ and cannot for $-1 <= alpha < 0$; and Grotto and Pappalettera [7, Prop. 2.1] characterize the self-similar motions and write the collapsing spiral, as a step toward bursts out of arbitrary configurations. We have not found a bound on the winding, the spiral angle or the distance traveled in the literature. In the Euler case the closed forms of the rates go back to [3] and [11], and the bound $P > sqrt(3)\/2$ with its minimum for each ratio of circulations is in [8]; Theorem 1 gives, for $alpha = 0$, a shorter proof of that bound through the side lengths.
...
+ T. Yasunaga, N. Otobe and T. Iwayama, _J. Phys. Soc. Jpn._ *90* (2021) 124401.
