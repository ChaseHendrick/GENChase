...
Let $\alpha > -1$. Every self-similar collapse of three point vortices under \eqref{eq:alpha} has $P > B(\alpha)$, and for every $\delta > 0$ there is one with $P < B(\alpha) + \delta$.
\end{theorem}
...
In the extremal limit $\rho \to 0$ the circulations \eqref{eq:circalpha} are, up to a factor, $\Gamma_2 \approx 1$ at the vertex opposite the shortest side and $\Gamma_1 \approx -\Gamma_3$ of order $\rho^{2\beta - 1}$ at its ends: two weak vortices of opposite sign, at distance $\rho$, spiral into a strong vortex, and the line through them makes the angle $\arccos(1/\sqrt{2(1 + \beta)})$ with the line to the strong vortex. For $\alpha = 0$ this is the limit $\mu \to 0$ of Corollary~\ref{cor:bound}, with the angle $60^\circ$ of Theorem~\ref{thm:pairs}.

\begin{corollary}\label{cor:geom}
Let $\alpha > -1$. In every self-similar collapse of three point vortices under \eqref{eq:alpha}, each vortex travels more than $(4 + \alpha)/(2 + \alpha)$ times its initial distance from the collision point, the angle between its path and the direction to that point exceeds $\arccos((2 + \alpha)/(4 + \alpha))$, and $|\omega_0| t_c > 2\sqrt{3 + \alpha}/(2 + \alpha)^2$. None of these constants can be improved.
\end{corollary}
...
\end{align*}
and in these coordinates the Hessian of the Lagrangian of the minimization of $P$ on the three-dimensional tangent space of the constraints has the eigenvalues $1.106$, $6.289$ and $9.377$. Like the minimizers of part (c), it has one strong and three weak vortices. The minima are local: we have not proved that $P_4$, $P_5$ and $P_6$ are the least values of $P$ over all collapses of four, five and six vortices. For four vortices a numerical minimization from more than two hundred random starts ends at the configuration above, up to relabeling, and for five and six vortices $P_5$ and $P_6$ are the least values found by multistart searches.

\begin{theorem}\label{thm:norot}
Under \eqref{eq:alpha} with $\alpha = 2$ there is a self-similar collapse of eleven point vortices with $\kappa$ real, so that $P = 0$: every vortex moves on a straight line into the collision point, and $|z_j(t) - z_c|^4 = (1 - t/t_c)\,|z_j(0) - z_c|^4$. In it all circulations and their sum are nonzero, no two vortices coincide and the angular impulse vanishes. Near it, the self-similar collapses of eleven vortices with $\kappa$ real form, modulo translations, rotations and the scalings of lengths and of circulations, a manifold of dimension nine.
\end{theorem}

In the normalization of the program, $2\pi\kappa = -1$, so that $t_c = \pi/2$, the collision point is the origin, one circulation is $1$ and nine others are fixed rational numbers. The configuration has one strong vortex of circulation $27.6\ldots$, the one closest to the collision point, one of circulation $-8.94\ldots$ and nine positive vortices with circulations between $0.37$ and $2.3$; the total circulation is $27.74\ldots$, the distances from the collision point lie between $0.21$ and $5.08$, and no two vortices are closer than $0.3487$. By Theorem~\ref{thm:alpha} three vortices cannot collapse without rotation in any $\alpha$-model with $\alpha > -1$, and no Euler collapse without rotation is known for any number of vortices (Section~\ref{sec:open}).
...
\caption{Numerical checks and computer-assisted proofs of the results.}\label{tab:checks}\\
  ...
  Remark~\ref{rem:below}: interval subdivision of $0.2622 \le \beta \le 1/2$, $3/2 \le m \le 2$, and minimization of $P - B(\alpha)$ for $-1.8 \le \alpha \le -1$ with precision growing with $|\ln\rho|$
    & 1205 boxes, all resolved; the minimum is positive at $\alpha = -1, -1.2, -1.5, -1.8$ \\ \hline
  ...
  Theorem~\ref{thm:norot}: Krawczyk operator with $2\pi\kappa = -1$ and nine circulations fixed
  ...
  The two-arm family, $N = 9, \dots, 603$: fit over $N \ge 301$ and extrapolation in $1/N$
    & $0.477360 + 1.2206/N + 2.93/N^2$, rms $7 \times 10^{-10}$; limit $0.47736$ \\ \hline
  SQG collapse of sixty vortices with $\kappa$ held real, Newton's method at 60 digits
...
\medskip
{\raggedright\noindent\textbf{Data availability.} The programs \texttt{verify\_general\_mu.py}, \texttt{verify\_floors\_independent.py}, \texttt{verify\_direct\_proof.py}, \texttt{verify\_central\_vortex.py}, \texttt{verify\_strong\_vortex.py}, \texttt{verify\_alpha\_winding.py}, \texttt{verify\_alpha\_extension.py}, \texttt{verify\_alpha\_equal\_circulations.py}, \texttt{verify\_many\_vortices.py} and \texttt{certify\_collapses.py} with its modules, used for Sections~\ref{sec:more} and~\ref{sec:numerics}, and \texttt{plot\_minimal\_winding.py} and \texttt{plot\_alpha\_winding.py}, used for Figures~\ref{fig:minima}--\ref{fig:alpha}, and their output are in the repository \url{https://github.com/ChaseHendrick/minimal-winding}: the programs in \texttt{code/}, the output of the ten verification programs and the stored configurations of Section~\ref{sec:more} in \texttt{data/}, and the figures in \texttt{paper/figures/}.\par}
