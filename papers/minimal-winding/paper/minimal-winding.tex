...
By Theorem~\ref{thm:alpha} three vortices cannot collapse without rotation in any $\alpha$-model with $\alpha > -1$, and no Euler collapse without rotation is known for any number of vortices (Section~\ref{sec:open}).
...
\item Does Theorem~\ref{thm:alpha} hold for $-2 < \alpha < -59/40$? Numerical minimization at $\alpha = -1.5$ and $-1.8$ finds no counterexample.
