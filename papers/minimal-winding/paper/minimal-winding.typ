...
#v(0.5em)
#par(justify: false)[*Data availability.* The programs `verify_general_mu.py`, `verify_floors_independent.py` and `plot_minimal_winding.py`, used for Section 5 and @fig:minima, and their output are in the repository https://github.com/ChaseHendrick/minimal-winding: the programs in `code/`, the output of the two verification programs in `data/`, and the figure in `paper/figures/`.]
