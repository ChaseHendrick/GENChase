...
For $Gamma_0 = 0$ the root greater than $1$ is $x_n = e^eta$, so $t = eta\/2$. Since $n = (n - 1) cosh eta$ and $(n - 1) sinh eta = sqrt(2n - 1)$, we get $b = coth(eta\/2) = sqrt(2n - 1)$ and $a = (n - 1) sinh((n + 2) eta \/ 2) = K_n$, so @eq:Pcentre is @eq:Pring and the minimum is $F_n$ of Proposition 2; in particular $F_n > sqrt(3)\/2$ for every $n$. The family and its rates are those of Demina and Kudryashov [7, Sect. 3, Eqs. (36)–(37)]: their Eq. (37) with $Gamma_1 = r^2 = x$ is $x$ times @eq:circ0, and on @eq:circ0 their constant $Omega$ is $S$. For $n = 2$ the configuration is the five-vortex collapse of Novikov and Sedov [22], a parallelogram with a vortex where its diagonals cross, and Gotoda gives its rates in closed form [10], in his Eq. (3.13) in arXiv:2002.09624v1. These sources give the rates, but none of them minimizes or bounds their ratio. Proposition 3 does not conflict with the seven-vortex collapse with $P = 0.8053 dots$ in Section 3, which has three pairs of vortices around the central one, not two regular polygons.

*Remark 4* (A leading-order calculation). In Corollary 1 the value $sqrt(3)\/2$ is approached as $mu -> 0$, and in Proposition 3 as $Gamma_0 -> infinity$; in both limits a strong vortex carries weak, tight pairs of opposite sign. The following formal calculation, to leading order and not a theorem, shows why the constant is the same. Place a vortex of circulation $1$ at the origin and, for $k = 1, dots, m$, a vortex of circulation $gamma_k > 0$ at $Z_k$ and one of circulation $-gamma_k + sigma_k$ at $Z_k (1 + gamma_k u_k)$, and let the $gamma_k$ tend to $0$ at the same rate, with $sigma_k = O(gamma_k^2)$. To leading order, the separation within each pair shrinks and turns at the common rate $kappa$ only if $u_k = 1\/2 + i y_k$ and $sigma_k = gamma_k^2$; then $-gamma_k + gamma_k^2$ agrees with the three-vortex harmonic value $-gamma_k \/ (1 + gamma_k)$ to this order, the angular impulse of the pair about the strong vortex vanishes to leading order, $kappa = i overline((1 + 1\/u_k)) \/ (2 pi |Z_k|^2)$, the motion collapses for $y_k > 0$, and

$ P -> (y_k^2 + 3\/4) / (2 y_k) = sqrt(3)/2 + (y_k - sqrt(3)\/2)^2 / (2 y_k) >= sqrt(3)/2 . $

Equality holds only if every $u_k = e^(i pi \/ 3)$ and all the pairs are at the same distance from the strong vortex: each pair then has length $gamma_k |Z_k|$ and makes the angle $60 degree$ with the direction away from the strong vortex. The minimizing configurations of Theorem 1 as $mu -> 0$, where the pair is formed by vortices $2$ and $3$ (Figure 2(b)), and of Proposition 3 as $Gamma_0 -> infinity$, where the $n$ pairs are the vortices at $epsilon^k$ and $sqrt(x) e^(i theta) epsilon^k$ with $gamma_k = x \/ Gamma_0$, both reduce to this calculation, which is why $sqrt(3)\/2$ appears in both. The seven-vortex collapse of Section 3 lies outside this regime: no vortex dominates, and its pairs are not tight.

= Numerical verification

The results were checked numerically as summarized in @tab:checks. Except where the table says otherwise, the checks evaluate the Biot–Savart velocities @eq:bs directly for configurations built independently of @eq:pos (two vortices fixed and the third moved around the zero-impulse circle), test self-similarity by comparing $dot(z)_j \/ (z_j - z_c)$ across all vortices, and obtain $P$ from @eq:P. The algebraic steps in the proofs were verified in exact arithmetic (SymPy): Lemma 3 for all three vortices, @eq:K, the resultant in the proof of Theorem 1(b), the discriminant and the resultant in the proof of Theorem 1(c), the irreducibility of $Q$ and of each of the 277 sextics in Remark 1, the constants in Proposition 1, and, for general $n$, the identities of Section 4 from @eq:circ to @eq:Pring, together with the table of constants. The program `verify_general_mu.py` carries out these exact computations with SymPy 1.14.0 and the high-precision checks with mpmath 1.3.0; `verify_floors_independent.py` repeats the minimizations independently and contains the exact sign changes of $G$ and the interval enclosures in the proof of Proposition 1, computed with the interval arithmetic of mpmath at 30 digits. The program `verify_direct_proof.py` checks every identity in the direct proof of Corollary 1 exactly, with SymPy. The program `verify_central_vortex.py` checks Proposition 3 and Remark 4: exactly, with SymPy, the derivation of @eq:circ0 and @eq:Pcentre for symbolic $n$, every identity in the proof, the case $Gamma_0 = 0$, the expansion and the leading-order calculation; numerically, the Biot–Savart velocities and the minima over $theta$ at 50 and 30 digits; and negative controls, checks that must fail and do. Each program runs in under a minute under Python 3.11 and prints every check with its result; their output is in `data/`.
