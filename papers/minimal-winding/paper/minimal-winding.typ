...
Three point vortices collapse self-similarly only if $1\/Gamma_1 + 1\/Gamma_2 + 1\/Gamma_3 = 0$ and their angular impulse about the center of vorticity vanishes (Lemma 2 below). Under these conditions every configuration moves self-similarly: the triangle keeps its shape and rotates while it shrinks to a point in finite time, expands, or rotates rigidly [9, Sect. 4.5.2], [16]. Such collapse was found by Gröbli [11] and found again by Aref [1] and by Novikov and Sedov [20], who also constructed collapsing configurations of four and five vortices; see [4] for the history and [19] for the general theory. Synge [23, Sect. 4, Theorem 8] identified $Gamma_1 Gamma_2 + Gamma_2 Gamma_3 + Gamma_3 Gamma_1 = 0$ as an exceptional case: the configurations whose shape lies on a certain hyperbola in his trilinear coordinates then come in one-parameter families of similar triangles, so that the shape does not determine the size. Conte and de Seze [6] solved the motion of three vortices of arbitrary circulations exactly and showed that when $sum_(j < k) Gamma_j Gamma_k = 0$ and the angular impulse vanishes, each vortex runs a logarithmic spiral about the center of vorticity while the triangle keeps its shape, and the configuration either collapses in finite time or expands [6, Sect. 4]. Kimura [14] studied similarity solutions of point-vortex systems, Aref [3] derived formulas for the rate of collapse or expansion and the angular frequency of rotation, and Gotoda [10] gave explicit formulas for the self-similar motions of three vortices. Borisov and Lebedev [5] obtained conditions for the collapse and the scattering of three vortices within the Lie–Poisson formulation of the problem, and Krishnamurthy, Aref and Stremler [16] recast the motion of three vortices in terms of the circumcircle and the interior angles of the vortex triangle and derived equations of motion for these quantities. Collapse also occurs in configurations of higher symmetry. Aref [2] reduced the motion of two concentric regular $n$-gons of vortices to an integrable Hamiltonian system with two degrees of freedom, and Koiller et al. [15] found collapsing configurations of two such rings, whose vortices move on logarithmic spirals. Demina and Kudryashov [7, Sect. 3] gave explicitly a family of such configurations that collapse or scatter, with an additional vortex, possibly of zero circulation, at the center, together with the complex constant that determines their rates of collapse and rotation.
...
_Proof._ The harmonic condition and the vanishing of the angular impulse are the classical necessary conditions [3, Sect. II B] (see also [14, Eqs. (3.20)–(3.21), (3.33)], [5, Sect. 3] and, for both conditions and any number of vortices, [7, Eqs. (26)–(28)]); we include the short argument. Along the motion
...
Every collapse of three point vortices in the plane is self-similar [8, Theorem 1.1], [9, Prop. 5.2], [17], and every motion of three vortices that ends in a total collision is self-similar [13, Theorem 1], so the bound of Corollary 1 holds for every collapse of three point vortices. The bound does not carry over to larger systems: the collapsing configuration of seven vortices in [7, Table 1, Fig. 1a] has the constant $Omega = 12433\/9000 - (31 sqrt(155)\/450) i$, where $Omega = 2 pi i overline(kappa)$ [7, Eq. (11)], so $P = |op("Re") Omega| \/ (2 |op("Im") Omega|) = 12433\/(1240 sqrt(155)) = 0.8053 dots < sqrt(3)\/2$.
...
*Remark 2.* For $mu = 1$, that is $Gamma = (1, 1, -1\/2)$, the zero-impulse circle is $|w - 1\/2| = sqrt(3)\/2$. With $w = 1\/2 + (sqrt(3)\/2) e^(i beta)$, $z_1 = 0$ and $z_2 = 1$, the same computation gives $P = (3 - cos 2 beta) \/ (2 sin 2 beta)$ on the collapsing arcs $0 < beta < pi\/2$ and $pi < beta < 3 pi\/2$, which are exchanged by interchanging the two equal vortices ($w -> 1 - w$). Since $P$ has period $pi$ in $beta$, Lemma 1 applies on both arcs and gives $P >= sqrt(2)$, with equality if and only if $cos 2 beta = 1\/3$, in agreement with Theorem 1. This parametrization is Kimura's [14, Sect. 4], who gives the two rates for $Gamma = (2, 2, -1)$ in his Eq. (4.4); their ratio $B\/(-2A)$ is this expression for $P$, which is also a specialization and reparametrization of Gröbli's spiral coefficient [11, Sect. 10].
...
For $n = 2$ the configuration is a parallelogram of four vortices; four-vortex collapse is treated by Novikov and Sedov [20, Sect. 4]. The ratio $x_n$ is rational exactly when $2n - 1$ is a perfect square, and $n = 5$ is the smallest such $n$. There $x_5 = 2$ and
...
#figure(
  text(size: 9pt, table(
    ...
    [$mu = 1$: Kimura's rates $A$, $B$ [14, Eq. (4.4)] at ten angles in $0 < beta < pi\/2$, 50 digits], [$kappa = (A + i B)\/(4 pi)$ for $Gamma = (1, 1, -1\/2)$, to $10^(-45)$],
...
Lemma 3 gives, in a form suited to minimization, the collapse and rotation rates that Aref [3, Eqs. (25a), (25d)] expresses through the side lengths of the triangle (see also [10], [9, Eq. (E.1)], and, for equal circulations, [14, Eq. (4.4)]); Aref also notes that the product of the two rates fixes the logarithmic spiral [3, Eq. (29c)]. Theorem 1 concerns the minimization of that product, which neither [3], [6] nor [14] considers. When the two circulations of the same sign differ, $mu < 1$, the two orientations of the triangle are not equivalent and the minima on the two arcs differ, so a minimization restricted to one orientation finds only one of them. The infimum $sqrt(3)\/2$ of Corollary 1 is approached only as $mu -> 0$ and is not attained.

Leoncini, Kuznetsov and Zaslavsky [18] analyze the motion near collapse when two of the vortices are identical; the configuration that collapses fastest at a fixed distance between the identical vortices, found by Kimura [14, Eq. (4.6)] and shown in [18, Fig. 18], has $P = 3\/2$, and the value $sqrt(3)\/2$ quoted there is that of their energy parameter $Lambda = e^(4 pi H)$, not of $P$. Tavantzis and Ting [24] describe, for $sum_(j < k) Gamma_j Gamma_k = 0$, the contracting and the expanding self-similar solutions as one-parameter families and discuss their stability. Reinaud, Dritschel and Scott [22] extend the collapse conditions to generalized Euler and quasi-geostrophic models and map the collapse time over the collapsing configurations, with the distance between the like-signed vortices fixed, finding local minima of that time; $P$, unlike the collapse time, does not depend on the choice of length and time scales. Krishnamurthy and Stremler [17] relate the interior angles of the triangle, the circulation ratios, the energy, the collapse time and the distance traveled before collapse by the circumcenter of the triangle. When the angular impulse about the center of vorticity vanishes, the circumcircle passes through the center of vorticity at all times [16, Eq. (40)], so in a collapse the circumcenter stays one circumradius from the collision point; since it is carried by the self-similar motion, it travels $sqrt(1 + 4P^2)$ times the initial circumradius before the collapse. They observe numerically that this normalized distance exceeds $2$ [17, Sect. 3.5]; Corollary 1 proves this observation and shows that the constant $2$ cannot be improved.
