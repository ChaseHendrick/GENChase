...
def clip(M):
    return arb_mat([[M[i, j] if i != j else arb(0) for j in range(4)] for i in range(4)]) if True else M
...
def run(cname, c, tmax=260, verbose=True):
    ...
    while t < tmax:
        ...
        y = [arb(v.mid()) for v in ph]
        z = [arb((znew[i, 0] + ph[i].rad()).upper()) for i in range(4)]
        Umax = Umax.union(hull[0] + arb(0, eta[0])) if nstep else hull[0] + arb(0, eta[0])
        t += h; nstep += 1
