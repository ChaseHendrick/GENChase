...
# Cross-check with det(lam I - J) at a test value (symbolic identity checked in arb)
def J(k):
    return arb_mat([[-k, -k, k, 0], [eps * k, 0, 0, 0], [0, 0, 0, 1], [-s, 0, 1, 0]])
lt = arb(fmpq(3, 7))
M = arb_mat([[lt if i == j else 0 for j in range(4)] for i in range(4)]) - J(kap)
print("det(lam I - J) - p(lam) at lam=3/7 contains 0:", (M.det() - p(lt)).contains(0))
