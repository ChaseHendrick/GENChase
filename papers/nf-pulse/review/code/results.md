...
| M29_eps_float_0p1 | nfcore.py | eps from the double 0.1 (= 0.1 + 5.55e-18) | 1 | R rest state unique, s = S'(0) < 1, one unstable and three sta; R negative control theta = 0 (S'(0) = 5) is refused; M unstable manifold validated for |t| <= 1 (c1); M unstable manifold validated for |t| <= 1 (c2); M unstable manifold validated for |t| <= 1 (interval); M negative control sigma x 8 is refused; B block around rest certified (|U| <= 0.05); B negative control U up to 0.15 is refused; J Jacobian with d/dkappa against finite differences (a test, n; P every orbit with c in [c1, c2] is in the interior of the blo; P the orbit at c1 enters the cone K- inside the block; P the orbit at c2 enters the cone K+ inside the block; N negative control, the orbit at c1 asked to reach K+, is refu; N negative control c = 1.1024, far from the pulse speed, is re |
