...
--mode       ${Object.keys(MODES).join(', ')}, metal, derive, vortex-collapse, vortex-grow, ${ART_MODES.join(', ')}, ${Object.keys(EXPERIMENTS).join(', ')}
...
function parse(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    const key = { '--mode': 'mode', '--machine': 'machineSlug', '--id': 'id', '--workspace': 'workspace', '--grid': 'grid', '--steps': 'steps', '--n': 'n', '--samples': 'samples', '--slug': 'slug', '--alpha': 'alpha', '--start': 'start', '--power': 'power',
    ...
    else input[key] = ['grid', 'steps', 'n', 'samples', 'alpha', 'start', 'keep', 'inches', 'ppi', 'budget', 'generations'].includes(key) ? Number(value) : value;
  ...
  if (!explicitWorkspace && (['metal', 'derive', 'vortex-collapse', 'vortex-grow', ...ART_MODES].includes(input.mode) || EXPERIMENTS[input.mode])) input.workspace = 'contribute';
  return { input, resume };
}
