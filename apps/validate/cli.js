...
--mode       ${Object.keys(MODES).join(', ')}, metal, derive, vortex-collapse, vortex-grow, vortex-threshold, ${Object.keys(EXPERIMENTS).join(', ')}
...
function parse(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    const key = { '--mode': 'mode', '--machine': 'machineSlug', '--id': 'id', '--workspace': 'workspace', '--grid': 'grid', '--steps': 'steps', '--n': 'n', '--samples': 'samples', '--slug': 'slug', '--alpha': 'alpha', '--start': 'start', '--threads': 'threads', '--to': 'to', '--power': 'power' }[flag];
    if (!key || !args[i + 1] || args[i + 1].startsWith('--')) throw Error('Unknown or missing option: ' + flag);
    const value = args[++i];
    if (key === 'power') input.power.mode = value;
    else input[key] = ['grid', 'steps', 'n', 'samples', 'alpha', 'start', 'threads', 'to'].includes(key) ? Number(value) : value;
  ...
  if (!explicitWorkspace && (['metal', 'derive', 'vortex-collapse', 'vortex-grow', 'vortex-threshold'].includes(input.mode) || EXPERIMENTS[input.mode])) input.workspace = 'contribute';
  return { input, resume };
}
