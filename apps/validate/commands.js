...
function command(root, input) {
  if (!input || Object.keys(input).some(k => !['workspace', 'mode', 'id', 'slug', 'n', 'samples', 'grid', 'steps', 'power', 'machineSlug'].includes(k))) throw Error('Unsupported job settings.');
  ...
  if (input.workspace !== 'contribute') throw Error('Choose Validate or Contribute.');
  if (input.mode === 'metal') {
  ...
  if (input.mode !== 'derive') throw Error('Choose a supported Contribute job.');
