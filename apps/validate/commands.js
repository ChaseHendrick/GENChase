...
function command(root, input) {
  if (!input || Object.keys(input).some(k => !['workspace', 'mode', 'id', 'slug', 'n', 'samples', 'grid', 'steps', 'alpha', 'start', 'threads', 'to', 'power', 'machineSlug', 'shareAutomatically'].includes(k))) throw Error('Unsupported job settings.');
