...
const ART_INPUT = { id: 'art-id', recipe: 'art-recipe', steps: 'art-steps', grid: 'art-grid', samples: 'art-samples', start: 'art-start', keep: 'art-keep', inches: 'art-inches', ppi: 'art-ppi', budget: 'art-budget' };
function artInput(mode) {
  const text = id => $(id).value.trim(), number = id => Number(text(id)), out = { inches: number('art-inches'), ppi: number('art-ppi') };
