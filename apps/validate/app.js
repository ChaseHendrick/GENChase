...
function paint() {
  ...
  if (active && j && syncedJob !== j.id) {
    ...
    const fields=['vortex-collapse','vortex-grow','vortex-threshold'].includes(j.input.mode)?{alpha:'vortex-alpha',n:'vortex-n',samples:'vortex-seeds',start:'vortex-start',threads:'vortex-threads',to:'vortex-to'}:{id:'technique',slug:'slug',n:'order',samples:'samples',grid:'gpu-grid',steps:'gpu-steps'};
...
async function action(name) {
  ...
  try {
    const input = name === 'start' ? { workspace, shareAutomatically: $('share-auto').checked, machineSlug: $('machine-slug').value, mode: $('mode').value, power: powerSettings(), ...(workspace === 'validate' ? (['technique','plate','print','witness'].includes($('mode').value) ? { id: $('technique').value } : {}) : $('mode').value === 'derive' ? { slug: $('slug').value, n: Number($('order').value), samples: Number($('samples').value) } : $('mode').value==='metal'?{grid:Number($('gpu-grid').value),steps:Number($('gpu-steps').value)}:$('mode').value==='vortex-threshold'?{alpha:Number($('vortex-alpha').value),n:Number($('vortex-n').value),to:Number($('vortex-to').value)}:['vortex-collapse','vortex-grow'].includes($('mode').value)?{alpha:Number($('vortex-alpha').value),n:Number($('vortex-n').value),samples:Number($('vortex-seeds').value),threads:Number($('vortex-threads').value),...($('vortex-start').value===''?{}:{start:Number($('vortex-start').value)})}:{}) } : {};
