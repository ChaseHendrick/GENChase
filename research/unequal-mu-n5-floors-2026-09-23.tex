...
% Build from this directory: pdflatex unequal-mu-n5-floors-2026-09-23.tex (twice).
...
\begin{proof}
The harmonic condition and the vanishing of the angular impulse are the classical necessary conditions \cite[Sect.~II~B]{aref2010}; we include the short argument. Along the motion
\[
...
\begin{table}[t]
  ...
  \begin{tabular}{|p{0.53\textwidth}|p{0.37\textwidth}|}
    ...
    Lemma~\ref{lem:rates} against the Biot--Savart velocities of all three vortices at the positions \eqref{eq:pos}, 36 random pairs $(\mu, \theta)$, 50 digits
      & relative difference $\le 10^{-50}$ \\ \hline
    Arc minima by direct minimization of $P$, 26 values of $\mu \in (0, 1]$ at 30 digits and 12 rational values at 50 digits
      & agree with the roots of $Q(\mu, \cdot)$ to $6 \times 10^{-25}$ and $5 \times 10^{-45}$ \\ \hline
    ...
    $\mu = 1/2$: integration of \eqref{eq:bs} from the minimizing configuration on $\Am$ (Taylor method, 30 digits)
      & $|z_j - z_c|^2$ follows $1 - t/t_c$ to 30-digit working precision up to $t = 0.9\, t_c$ \\ \hline
