...
- `python3 research/verify_general_mu.py` checks the general-μ theory and everything else the paper says was checked, and exits with an error if any check fails. It runs 116 checks in about 30 s, and its output is in `artifacts/verify-general-mu-2026-09-23.txt`. The output also contains an argument not used in the paper: Q(μ, ·) is irreducible for every rational μ ≠ 1, shown via an elliptic curve of rank 0. That argument has not been refereed.
