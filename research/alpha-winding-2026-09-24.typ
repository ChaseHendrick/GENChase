...
The spiral form @eq:spiral of a self-similar collapse in the $alpha$-models is in [15] and [9, Eqs. (3)–(6)], in [5, (A.19)], whose Hölder exponent $1\/(alpha + 1)$ is our $1\/(2 beta)$, and in [7, Prop. 2.1]. For $beta = 1$, $P = |omega_0| t_c$ is the product studied in [8]. For $beta != 1$ the product $|omega_0| t_c$ is the rotation per unit decrease of $ln (t_c - t)$, and it differs from $P$ by the factor $1\/beta$; we state the bound for $P$, which measures the geometry of the spiral, and for $|omega_0| t_c$ in Corollary 1.
