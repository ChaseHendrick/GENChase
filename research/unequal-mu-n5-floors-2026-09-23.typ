...
*Remark 2.* For $mu = 1$, that is $Gamma = (1, 1, -1\/2)$, the zero-impulse circle is $|w - 1\/2| = sqrt(3)\/2$. With $w = 1\/2 + (sqrt(3)\/2) e^(i beta)$, $z_1 = 0$ and $z_2 = 1$, the same computation gives $P = (3 - cos 2 beta) \/ (2 sin 2 beta)$ on the collapsing arcs $0 < beta < pi\/2$ and $pi < beta < 3 pi\/2$, which are exchanged by interchanging the two equal vortices ($w -> 1 - w$). Since $P$ has period $pi$ in $beta$, Lemma 1 applies on both arcs and gives $P >= sqrt(2)$, with equality if and only if $cos 2 beta = 1\/3$, in agreement with Theorem 1. This parametrization is Kimura's [11, Sect. 4], who gives the two rates for $Gamma = (2, 2, -1)$ in his Eq. (4.4); their ratio $B\/(-2A)$ is this expression for $P$, which is also a specialization and reparametrization of Gröbli's spiral coefficient [10, Sect. 10].
...
For $n = 2$ the configuration is a parallelogram of four vortices; four-vortex collapse is treated by Novikov and Sedov [17, Sect. 4]. The ratio $x_n$ is rational exactly when $2n - 1$ is a perfect square, and $n = 5$ is the smallest such $n$. There $x_5 = 2$ and
...
Lemma 3 gives, in a form suited to minimization, the collapse and rotation rates that Aref [3, Eqs. (25a), (25d)] expresses through the side lengths of the triangle (see also [9], [8, Eq. (E.1)], and, for equal circulations, [11, Eq. (4.4)]); Aref also notes that the product of the two rates fixes the logarithmic spiral [3, Eq. (29c)]. Theorem 1 concerns the minimization of that product, which neither [3] nor [11] considers. When the two circulations of the same sign differ, $mu < 1$, the two orientations of the triangle are not equivalent and the minima on the two arcs differ, so a minimization restricted to one orientation finds only one of them. The infimum $sqrt(3)\/2$ of Corollary 1 is approached only as $mu -> 0$ and is not attained.

Leoncini, Kuznetsov and Zaslavsky [15] analyze the motion near collapse when two of the vortices are identical; the configuration that collapses fastest at a fixed distance between the identical vortices, found by Kimura [11, Eq. (4.6)] and shown in [15, Fig. 18], has $P = 3\/2$, and the value $sqrt(3)\/2$ quoted there is that of their energy parameter $Lambda = e^(4 pi H)$, not of $P$. Tavantzis and Ting [20] describe, for $sum_(j < k) Gamma_j Gamma_k = 0$, the contracting and the expanding self-similar solutions as one-parameter families and discuss their stability. Reinaud, Dritschel and Scott [19] extend the collapse conditions to generalized Euler and quasi-geostrophic models and map the collapse time over the collapsing configurations, with the distance between the like-signed vortices fixed, finding local minima of that time; $P$, unlike the collapse time, does not depend on the choice of length and time scales. Krishnamurthy and Stremler [14] relate the interior angles of the triangle, the circulation ratios, the energy, the collapse time and the distance traveled before collapse by the circumcenter of the triangle. When the angular impulse about the center of vorticity vanishes, the circumcircle passes through the center of vorticity at all times [13, Eq. (40)], so in a collapse the circumcenter stays one circumradius from the collision point; since it is carried by the self-similar motion, it travels $sqrt(1 + 4P^2)$ times the initial circumradius before the collapse. They observe numerically that this normalized distance exceeds $2$ [14, Sect. 3.5]; Corollary 1 proves this observation and shows that the constant $2$ cannot be improved.
