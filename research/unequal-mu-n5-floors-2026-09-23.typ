...
Lemma 3 gives, in a form suited to minimization, the collapse and rotation rates that Aref [3, Eqs. (25a), (25d)] expresses through the side lengths of the triangle (see also [6]); Aref also notes that the product of the two rates fixes the logarithmic spiral [3, Eq. (29c)]. Theorem 1 concerns the minimization of that product, which [3] does not consider. When the two circulations of the same sign differ, $mu < 1$, the two orientations of the triangle are not equivalent and the minima on the two arcs differ, so a minimization restricted to one orientation finds only one of them. The infimum $sqrt(3)\/2$ of Corollary 1 is approached only as $mu -> 0$ and is not attained.

Leoncini, Kuznetsov and Zaslavsky [11] analyze the motion near collapse when two of the vortices are identical. Tavantzis and Ting [15] describe, for $sum_(j < k) Gamma_j Gamma_k = 0$, the contracting and the expanding self-similar solutions as one-parameter families and discuss their stability. Krishnamurthy and Stremler [10] relate the interior angles of the triangle, the circulation ratios, the energy, the collapse time and the distance traveled before collapse. In their normalization that distance is $sqrt(1 + 4P^2)$, and they observe numerically that it exceeds $2$ [10, Sect. 3.5]. Corollary 1 proves this observation and shows that the constant $2$ cannot be improved.
