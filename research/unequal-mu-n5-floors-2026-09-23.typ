...
Three point vortices collapse self-similarly only if $1\/Gamma_1 + 1\/Gamma_2 + 1\/Gamma_3 = 0$ and their angular impulse about the center of vorticity vanishes (Lemma 2 below). Under these conditions every configuration moves self-similarly: the triangle keeps its shape and rotates while it shrinks to a point in finite time, expands, or rotates rigidly [1, 13]. Such collapse was found by Gröbli [9] and found again by Aref [1] and by Novikov and Sedov [16], who also constructed collapsing configurations of four and five vortices; see [4] for the history and [15] for the general theory. Kimura [10] studied similarity solutions of point-vortex systems, Aref [3] derived formulas for the rate of collapse or expansion and the angular frequency of rotation, and Gotoda [8] gave explicit formulas for the self-similar motions of three vortices. Borisov and Lebedev [5] obtained conditions for the collapse and the scattering of three vortices within the Lie–Poisson formulation of the problem, and Krishnamurthy, Aref and Stremler [12] recast the motion of three vortices in terms of the circumcircle and the interior angles of the vortex triangle and derived equations of motion for these quantities. Collapse also occurs in configurations of higher symmetry. Aref [2] reduced the motion of two concentric regular $n$-gons of vortices to an integrable Hamiltonian system with two degrees of freedom, and Koiller et al. [11] found collapsing configurations of two such rings, whose vortices move on logarithmic spirals.
...
_Proof._ The harmonic condition and the vanishing of the angular impulse are the classical necessary conditions [3, Sect. II B] (see also [5, Sect. 3]); we include the short argument. Along the motion
...
Every collapse of three point vortices in the plane is self-similar [7, Theorem 1.1], [13], so the bound of Corollary 1 holds for every collapse of three point vortices.
...
*Remark 2.* For $mu = 1$, that is $Gamma = (1, 1, -1\/2)$, the zero-impulse circle is $|w - 1\/2| = sqrt(3)\/2$. With $w = 1\/2 + (sqrt(3)\/2) e^(i beta)$, $z_1 = 0$ and $z_2 = 1$, the same computation gives $P = (3 - cos 2 beta) \/ (2 sin 2 beta)$ on the collapsing arcs $0 < beta < pi\/2$ and $pi < beta < 3 pi\/2$, which are exchanged by interchanging the two equal vortices ($w -> 1 - w$). Since $P$ has period $pi$ in $beta$, Lemma 1 applies on both arcs and gives $P >= sqrt(2)$, with equality if and only if $cos 2 beta = 1\/3$, in agreement with Theorem 1. This expression for $P$ is a specialization and reparametrization of Gröbli's spiral coefficient [9, Sect. 10].
...
This is the circulation condition of Koiller et al. [11, Sect. 11]. It coincides with the classical condition $sum_(i < j) Gamma_i Gamma_j = 0$ for the $2n$ vortices, whose left side here equals $(n\/2)((n - 1) x^2 - 2 n x + (n - 1))$. Its roots are $x_n$ and $1\/x_n$, where
...
The collapse rate and the rotation rate as functions of the relative angle are given by Koiller et al. [11, Sect. 11]; @eq:Pring writes their ratio in closed form.
...
For $n = 2$ the configuration is a parallelogram of four vortices; four-vortex collapse is treated by Novikov and Sedov [16, Sect. 4]. The ratio $x_n$ is rational exactly when $2n - 1$ is a perfect square, and $n = 5$ is the smallest such $n$. There $x_5 = 2$ and
...
Lemma 3 gives, in a form suited to minimization, the collapse and rotation rates that Aref [3, Eqs. (25a), (25d)] expresses through the side lengths of the triangle (see also [8]); Aref also notes that the product of the two rates fixes the logarithmic spiral [3, Eq. (29c)]. Theorem 1 concerns the minimization of that product, which [3] does not consider. When the two circulations of the same sign differ, $mu < 1$, the two orientations of the triangle are not equivalent and the minima on the two arcs differ, so a minimization restricted to one orientation finds only one of them. The infimum $sqrt(3)\/2$ of Corollary 1 is approached only as $mu -> 0$ and is not attained.

Leoncini, Kuznetsov and Zaslavsky [14] analyze the motion near collapse when two of the vortices are identical; the configuration that collapses fastest at a fixed distance between the identical vortices [14, Fig. 18] has $P = 3\/2$, and the value $sqrt(3)\/2$ quoted there is that of their energy parameter $Lambda = e^(4 pi H)$, not of $P$. Tavantzis and Ting [19] describe, for $sum_(j < k) Gamma_j Gamma_k = 0$, the contracting and the expanding self-similar solutions as one-parameter families and discuss their stability. Reinaud, Dritschel and Scott [18] extend the collapse conditions to generalized Euler and quasi-geostrophic models and map the collapse time over the collapsing configurations, with the distance between the like-signed vortices fixed, finding local minima of that time; $P$, unlike the collapse time, does not depend on the choice of length and time scales. Krishnamurthy and Stremler [13] relate the interior angles of the triangle, the circulation ratios, the energy, the collapse time and the distance traveled before collapse by the circumcenter of the triangle. When the angular impulse about the center of vorticity vanishes, the circumcircle passes through the center of vorticity at all times [12, Eq. (40)], so in a collapse the circumcenter stays one circumradius from the collision point; since it is carried by the self-similar motion, it travels $sqrt(1 + 4P^2)$ times the initial circumradius before the collapse. They observe numerically that this normalized distance exceeds $2$ [13, Sect. 3.5]; Corollary 1 proves this observation and shows that the constant $2$ cannot be improved.

For two rings, Aref [2] derived the reduced equations @eq:rings for arbitrary circulations and analyzed equal and opposite circulations, $x = 1$, which do not satisfy @eq:circ. Koiller et al. [11] found the collapsing configurations and their rates; @eq:Pring writes the ratio of these rates in closed form, and Proposition 2 gives its minimum over the relative rotation. O'Neil [17] proves that for generic circulations three concentric rings have finitely many relative equilibria and collapse configurations. Demina and Kudryashov [6] study relative equilibria, collapse and scattering of point vortices with arbitrary circulations and give explicit double-ring configurations formed by two regular polygons. We have not found the minimal values of Theorem 1 and Propositions 1 and 2 stated in the literature, nor a proof of the bound of Corollary 1 or of its sharpness.
