...
static int N, D;
static double A;  /* exponent */
static int A2;    /* 2A, an integer */
...
static iv d1mA2(iv d) { return iv_mul(d, dmA2(d)); }
  ...
  for (int j = 0; j < N; j++)
    for (int k = j + 1; k < N; k++) {
      ...
      if (t->pos[j][k]) {
        ...
        t->u[j][k] = t->u[k][j] = (A2 == 4) ? ivp(1.0) : d1mA2(t->d[j][k]);
...
static void evalE(const box *b, iv E[], iv J[][MAXD]) {
  ...
  for (int j = 0; j < N; j++) for (int k = 0; k < N; k++) if (j != k) {
    ...
    iv rd = iv_recip(dd);
    iv Aa = iv_mul(iv_scale(a, A), rd), Ab = iv_mul(iv_scale(bb, A), rd);
    /* 1 - Aa*w */
  ...
  for (int c = 0; c < D; c++) {
    ...
    for (int j = 0; j < N; j++) for (int k = j + 1; k < N; k++) {
      ...
      dU = iv_add(dU, iv_scale(iv_mul(t.p[j][k], comp), (2.0 - A) * e));
    }
    int q = 0;
    for (int j = 1; j < N - 1; j++) {
      civ dS = czero();
...
int main(int argc, char **argv) {
  ...
  iv_init();
  N = atoi(argv[1]); D = 2 * N - 4; A = atof(argv[2]); A2 = (int)(2 * A + 0.5);
  if (fabs(A2 - 2 * A) > 1e-12 || A < 2) { fprintf(stderr, "A must be a multiple of 1/2, >= 2\n"); return 2; }
  ...
  printf("STAT A=%g sym=%d N=%d worker=%d partitions=%d boxes=%ld t0_chart=%ld t2_H=%ld t3_partition=%ld t4_mv_krawczyk=%ld cert=%ld unres=%ld\n",
         A, use_sym, N, worker, nparts, st_boxes, st_t0, st_t2, st_t3, st_t4, st_cert, st_unres);
  return 0;
}
