...
- **Signed release tags.** `node tools/tag-release.js vX.Y.Z` makes an annotated, SSH-signed tag on `main` under the project identity after the release checks, and `--verify` checks any tag against `identities/allowed_signers`. The release workflow publishes an existing signed tag after verifying it, and refuses unsigned releases once a key is registered; a tag signature workflow checks every pushed `v*` tag. `docs/SIGNING.md` is the one-time key setup. Self-tests with negative controls for both tools run in `npm test`.
