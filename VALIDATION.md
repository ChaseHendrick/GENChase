...
> 68 unvalidated; 6 partially validated; 56 validated within stated limits.
...
| [grains](src/modules/grains.js) | partially validated | [grains-science.js](tools/grains-science.js) | Limited evidence recorded | none |
...
| [veselago](src/modules/veselago.js) | partially validated | [veselago-science.js](tools/veselago-science.js) | Limited evidence recorded | none |
...
- Partially validated, not validated: two criteria fixed before the run were missed. Production-step restitution reached -10.6% against a 10% bound, and incline slip at tan θ/3μ = 1.05 reached +8.67% against 3%. Both misses are reported in validation/GRAINS.md.
- Catalog equation and citation are review targets, not verified paper equivalence. The displayed equation of motion omits the background drag −c m v and −c I ω (c = ζ√(g/r0)/2) that grains.js applies to every grain; it does not enter the static balance.
- Complete evidence covers only the documented domain in validation/GRAINS.md and the grains-science / grains-print-state results artifacts: rigid walls, no top load, the pour, arch, disks and column presets at seed cundall-1979.
- The settle is not time-accurate at its production step: the restitution coefficient depends on where in a step contact begins, −12.1% to +5.7% of the analytic value at damping ≤ 0.5 (the pre-registered 10% bound was missed); it converges at first order to the analytic contact law.
- The top load does not load the packing: at photo, strong and slip the lid carries at most 3.4% of the weight at any step and nothing at the end, so presets with a top load, and the periodic sides only they use, are outside the domain.
- The starting lattice overlaps (255 to 562 pairs, up to 1.29 r0), so every packing is prepared by an explosive decompression and settling, not a pour. The same seed settles to a different packing in Node and Chromium because Math.cos differs by one ulp between them; each engine reprints its own packing exactly.
- No Radjai force-distribution, Janssen, photoelastic or experimental claim: side-wall friction carries −6.2% to +7.0% of the weight depending on history, and the column preset carries −0.1% in its printed packing. Broader settings require additional independent evidence.
...
- The tab's frozen regions are the theorem's polar regions (a domino connected to the boundary through edge-adjacent dominoes of its own type, as Johansson 2005 states the definition of Jockusch, Propp and Shor); they equal an independent union-find on all 2,350 arctic plates. The earlier local test read 0.2778 +/- 0.0009 in the limit, 67 sigma above 1 - pi/4.
- The status line's per-plate error bar is the standard error of the free cells summed over 60 angular sectors, from their integrated autocorrelation time (window closed at the first non-positive autocorrelation, floored at 2, capped at 15). The scatter of the printed fraction over seeds is 0.83 to 0.91 of it at orders 40 to 320, inside the calibration band 0.75 to 1.33 fixed before the run, but conservative, overstating the scatter by 10 to 20 per cent with 95 per cent intervals that exclude 1: the truncated window leaves out a negative autocorrelation lobe and the four-fold mean pattern of the sector counts enters each plate's variance. The bar is not calibrated below order 40, although the UI starts at 8.
- The spread of the four polar regions, the bar it replaced, was too wide: the scatter was 0.68 to 0.88 of it because neighboring regions are anticorrelated (-0.28 at order 40 to -0.06 at 320); the estimator was chosen on a separate training family of 1,950 plates, not on the tool's seeds.
...
- The polar-region area and the axis radius agree with 1 - pi/4 and 1/sqrt(2) only after extrapolation in n with the stated form q_inf + A n^-2/3 + B n^-1 (0.2127 +/- 0.0013, -1.5 sigma; 0.7097 +/- 0.0016, +1.7 sigma); a two-parameter n^-2/3 fit over orders 113 to 320 gives +1.4 and -2.6 sigma, so the agreement holds at about 0.003 in area and 0.005 in radius, and at order 320 the polar area is still 0.030 above its limit, which the status line attributes to finite n.
...
- Since recipe v4 a rhombus is frozen when a chain of edge-adjacent rhombi of its own orientation joins it to the rim, the lozenge analogue of the Jockusch-Propp-Shor polar regions (as Johansson 2005 states them) adopted by analogy; Cohn, Larsen and Propp state their theorem for the height function, and the definition is not quoted from them. The module's flags equal an independent union-find over rhombi and the extreme level sets of the height function on all 2,720 limit-shape plates.
- At every size the tab offers an exact plate reads below the limit shape, about seven of its own error bars on a regular hexagon (radius 0.9085 at 12, 12, 12 and 0.9663 at 48, 48, 48; free area 0.7516 and 0.8473 against 0.9069), because the frozen regions reach one to one and a half tiles past the ellipse; the status line names this finite-size cause. Agreement with the limit holds only after extrapolation with the stated form q_inf + A n^-2/3 + B n^-1: free area 0.9005 +/- 0.0052 (-1.2 sigma) and radius 0.9949 +/- 0.0033 (-1.5 sigma) for regular hexagons 12 to 48; 0.866 +/- 0.032 and 0.984 +/- 0.020 for the 3:5:6 family, whose three-parameter fit has one degree of freedom. The two-parameter form moves the regular limits by 0.006 in area and 0.007 in radius, well inside 3 sigma of the limit either way.
- The n^(1/3) boundary scale behind that form is assumed by analogy with the Aztec diamond; the spread of the corner depth grows as n^0.248 +/- 0.030 over sizes 12 to 48, 2.8 sigma below 1/3, with depths of only 3 to 8 cells.
- Both per-plate error bars (integrated autocorrelation time over 60 angular sectors, the free area now included) are checked against the scatter over seeds at the eleven hexagons measured, which is 0.77 to 1.03 of the bar, inside the band 0.75 to 1.33 fixed before the run; they are about right at 40 and 48 a side and overstate the scatter by up to 30 per cent elsewhere, and the lowest ratio, 0.77 at 12, 20, 24, is near the lower edge of the band. Lopsided boxes such as 48, 48, 8 and all other shapes are outside the calibrated domain.
...
- Catalog equation and citation are review targets, not verified paper equivalence.
- Complete evidence covers only the documented domain in validation/CPU-DYNAMICS.md and the rigid-body-audit / tennis-print-state results artifacts.
- No exact flip-time, all-control, calibrated color or indefinite energy-preservation claim; RK4 is not exactly symplectic.
- Broader settings require additional independent evidence.
...
- Partially validated: the Penrose P3 and P2, Ammann–Beenker and dodecagonal tiling geometry is established for the 68 enumerated recipes and 11 print recipes in validation/TILINGS.md. Full status is withheld because of two defects in src/modules/tilings.js, which this review did not change (next two items).
- Defect, P3 matching arcs (src/modules/tilings.js:31): the thin-rhomb arcs sit around the acute corners, both in one family. On the module's own legal P3 tilings 127,405 of 542,129 shared edges (in all 25 P3 recipes; 1,539 of 6,567 in the arcs preset) join arcs of different families, and the thin arcs pair the wrong crossings. Positions match; colours and loops do not. Thin arcs around the obtuse corners, one per family, give zero mismatches. The drawn P3 arcs are not a valid Penrose decoration; P3 legality here rests on the de Bruijn window, the vertex census and that reference decoration.
- Defect, grout inset (src/modules/tilings.js:626): insetPoly offsets along the left edge normals, which point outward once the print flips y, so "Gap inset (grout)" enlarges every tile instead of leaving grout. Printed fill polygons cover 1.060 (rhombs, inset 0.012) to 1.262 (kilnfloor, inset 0.05) of the page instead of less than 1. The SVG polygons equal the validated tiles offset outward, within 0.005 px.
- Catalog equation and citation are review targets. The deflation rule "thick half → 2 thick + 1 thin" is checked by exact counts; the hat and spectre descriptions are not reviewed.
- Hat and spectre are outside the reviewed domain: only congruence to the module's own prototiles, single cover of each supertile and reflection or chirality counts were measured, against no published reference.
- Dodecagonal: de Bruijn duality and tiling validity only. The internal-space window is a necessary condition that a single phason flip passes; no matching rule or vertex-type set is claimed.
- Finite enumerated recipes of the sun-centred Penrose tiling, one Chromium build and a software renderer. Node and Chromium round one sun-wheel sine differently, so Penrose coordinates differ between them by a few ulps (at most 7.1e-15 in five P3 print recipes).
- PNG prints are checked for dimensions, nonblank content, determinism and preserved state, not pixel by pixel against the geometry; colour, jitter and grain are presentation.
...
- Catalog equation and citation are review targets, not verified paper equivalence.
- Evidence covers only the documented domain in validation/VESELAGO.md and the veselago-science / veselago-print-state results artifacts: the geometric ray trace, the drawn rays, the n = −1 image metric and the print path.
- The status label is not validated and is wrong over part of the slider domain. It reads "Veselago focus" whenever \|Δx\|/W < 0.12 (veselago.js:152): in 11,829 of the 16,468 in-domain sweep configurations where no ray crosses the axis behind the slab, including n = −2.20 to −2.00 at the default slab, and at n = +1. This defect blocks full validation.
- When the back face lies beyond the last drawn column (grid 128 or 144 with a thick slab and a distant source), the exit segment is drawn backwards from the back face to W − 2 (veselago.js:84–87). That inks cells no ray reaches: 1,767 of 2,368 such sweep configurations, 24,214 stray cells. These plates are outside the domain, and the defect blocks full validation.
- Transmitted geometric rays only: no Fresnel reflection, wave optics, evanescent-wave amplification (Pendry), absorption or dispersion. At n = −0.40, rays with \|sin θ\| > 0.4 have no transmitted ray and are skipped entirely at veselago.js:79, incident segment included, with nothing on the plate or status line to say so: 2 of 12, 4 of 28, 6 of 48 and 6 of 64 rays.
- Refocusing at n = −1 (slab0 + d inside, slab0 + 2L − d behind) is a geometric identity of the flat slab. It is recorded as a regression check, not a prediction.
- Hash-only indices between −0.43497 and −0.4 reach the 0.05 slope clamp (veselago.js:88) for near-grazing rays: 322.85 px vertex error at n = −0.43527. These indices are outside the domain.
