...
`TECHNIQUES.md` lists every technique with the URL hash that reconstructs its plate exactly and the papers it implements; `techniques.json` is the same thing for a program to read. Both are generated by `node tools/index.js` from `studio.html`, so they cannot disagree with the file. Run that command after adding or renaming a technique, then `node tools/lint.js`. Lint fails a catalog that is behind the file, a README or CITATION.cff whose count is wrong, and a TECHNIQUES.md missing a hash. The README narrative (the self-check table, the "what is new" bullets) is still written by hand: a new plate that measures something should get a row, not just a generated line.
