...
GENChase is a modular-source generative art studio distributed as one generated `studio.html`. Maintained source lives in `src/`; build it with `node tools/build.js`. There is no package to install and no app to wrap it in. Humans opening a pull request follow `CONTRIBUTING.md`; this file is the contract for the change itself.
...
`TECHNIQUES.md` lists every technique with the URL hash that reconstructs its plate exactly and the papers it implements; `techniques.json` is the same thing for a program to read; `llms.txt` is a short file that tells a language model to read those and skip `studio.html`. All three are generated by `node tools/index.js` from `studio.html`, so they cannot disagree with the file. The same command stamps the live count into README, CITATION.cff, RESEARCH.md, DESIGN-PLAN.md, the studio head, and `.github/description.txt`, and appends a RESEARCH row for any new id. Do not hand-edit those numbers. Build first, then run that command after adding or renaming a technique, then `node tools/lint.js`. Lint fails a catalog that is behind the file, a README or CITATION.cff whose count is wrong, a TECHNIQUES.md missing a hash, a familiarity bucket that is missing or invented, a RESEARCH.md table missing a tab, a GitHub description file whose count is wrong, and an `og.jpg` whose JPEG COM comment does not match the live count. The README narrative (the self-check table, the "what is new" bullets) is still written by hand: a new plate that measures something should get a row, not just a generated line. `RESEARCH.md` is a handwritten ledger of prior-art searches and physics claims. It is not generated, except for the count and any missing table row. Read it before searching the web for "has this been done". If you think you have a new identity, search the literature first (the closed form and the extremum); do not derive a formula a paper already states. If the search is clean, uniqueness-check it against the papers and write `IDENTITIES.md`; do not put a name on work that already exists. Derived identities live in `IDENTITIES.md`. The three-vortex collapse bound specializes Gröbli's 1877 formula. Its former personal name is retired; use descriptive titles and credit classical sources. Add a line the same day you search; do not re-run a search the ledger marks skip.
...
- Hand-edit generated `studio.html`. Edit `src/` and run the dependency-free builder.
