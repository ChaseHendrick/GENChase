...
`TECHNIQUES.md` lists every technique with the URL hash that reconstructs its plate exactly and the papers it implements; `techniques.json` is the same thing for a program to read; `llms.txt` is a short file that tells a language model to read those and skip `studio.html`. All three are generated by `node tools/index.js` from `studio.html`, so they cannot disagree with the file. Run that command after adding or renaming a technique, then `node tools/lint.js`. Lint fails a catalog that is behind the file, a README or CITATION.cff whose count is wrong, a TECHNIQUES.md missing a hash, a familiarity bucket that is missing or invented, and an `og.jpg` whose JPEG COM comment does not match the live count. The README narrative (the self-check table, the "what is new" bullets) is still written by hand: a new plate that measures something should get a row, not just a generated line. `RESEARCH.md` is a handwritten ledger of prior-art searches and physics claims. It is not generated. Read it before searching the web for "has this been done" or claiming a new law. Add a line the same day you search; do not re-run a search the ledger marks skip.
