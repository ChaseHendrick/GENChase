...
(async () => {
  ...
  try {
    ...
    await page.goto(origin);
    await page.waitForFunction(() => document.querySelector('#status').textContent.startsWith('Run complete'));
    ...
    const fixtureMiss = { jobId: app.jobs.current.id, file: 'a'.repeat(40) + '-ui-test-fixture.json', kind: 'witness-miss', status: 'miss', id: 'fixture', expected: 1, got: 2, recipeHash: '#fixture/seed', commit: 'a'.repeat(40) };
    ...
    assert.equal(await restricted.textContent('#workspace-title'), 'Run experiments', 'blocked storage does not break navigation');
