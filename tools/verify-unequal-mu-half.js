 ...
 *
 * Usage: node tools/verify-unequal-mu-half.js
 * Writes: validation/results/unequal-mu-half.json
...
const SQRT5201 = Math.sqrt(5201);

/**
 * Closed form for P⋆ itself (trigonometric Cardano of the irreducible cubic for P⋆²):
 *   P⋆ = sqrt( 605/324 + (7√5201/162) cos(arccos(245351√5201 / 5201²)/3) )
 * Unique positive root > 2 of the irreducible minpoly
 *   8748 x^6 - 49005 x^4 + 27794 x^2 + 18723 = 0
 */
function pStarClosedForm() {
  const shift = 605 / 324;
  const amp = (7 * SQRT5201) / 162;
  const arg = (245351 * SQRT5201) / (5201 * 5201);
  return Math.sqrt(shift + amp * Math.cos(Math.acos(arg) / 3));
}

function minpolySextic(x) {
  const x2 = x * x;
  const x4 = x2 * x2;
  const x6 = x4 * x2;
  return 8748 * x6 - 49005 * x4 + 27794 * x2 + 18723;
}

function minpolyCubicQ(q) {
  return 8748 * q * q * q - 49005 * q * q + 27794 * q + 18723;
}
...
function main() {
  ...
  // Closed-form P⋆ (trig Cardano) vs product-at-cosθ⋆
  const Pclosed = pStarClosedForm();
  const TARGET_DIGITS = 2.203855016036133;
  check('closed_form_matches_product', relErr(Pclosed, Pstar) < REL_TOL, {
    Pclosed,
    Pstar,
    rel: relErr(Pclosed, Pstar),
  });
  check('closed_form_ge12_digits_vs_target', Math.abs(Pclosed - TARGET_DIGITS) < 5e-13, {
    Pclosed,
    TARGET_DIGITS,
    absDiff: Math.abs(Pclosed - TARGET_DIGITS),
  });
  check('minpoly_sextic_residual', Math.abs(minpolySextic(Pstar)) < 1e-8, {
    // Float64 evaluation of degree-6 with O(10^4) coeffs; residual scale ~1e-10..1e-8
    residual: minpolySextic(Pstar),
    Pstar,
  });
  check('minpoly_cubic_P2_residual', Math.abs(minpolyCubicQ(Pstar * Pstar)) < 1e-9, {
    residual: minpolyCubicQ(Pstar * Pstar),
    P2: Pstar * Pstar,
  });
  check('closed_form_minpoly_residual', Math.abs(minpolySextic(Pclosed)) < 1e-8, {
    residual: minpolySextic(Pclosed),
    Pclosed,
  });
  ...
  const result = {
    ...
    closedForm: {
      ...
      Pstar:
        'sqrt(605/324 + (7√5201/162) cos(arccos(245351√5201/5201^2)/3))',
      minpolySextic: '8748 x^6 - 49005 x^4 + 27794 x^2 + 18723 = 0',
      minpolyCubicP2: '8748 q^3 - 49005 q^2 + 27794 q + 18723 = 0  (q=P⋆²)',
    },
    numbers: {
  ...
  const outPath = path.join(outDir, 'unequal-mu-half.json');
  fs.writeFileSync(outPath, JSON.stringify(result, null, 2) + '\n');

  const pstarPath = path.join(outDir, 'unequal-mu-half-Pstar.json');
  const pstarResult = {
    generated: result.generated,
    status: 'proved candidate; priority unconfirmed',
    claim: {
      PstarClosed:
        'sqrt(605/324 + (7*sqrt(5201)/162)*cos(arccos(245351*sqrt(5201)/5201^2)/3))',
      minpolySextic: '8748*x^6 - 49005*x^4 + 27794*x^2 + 18723 = 0',
      minpolyCubicP2: '8748*q^3 - 49005*q^2 + 27794*q + 18723 = 0',
      irreducibleOverQ: true,
      degree: 6,
      note: 'Casus irreducibilis for cubic in P⋆²; no simpler real radical over Q. Uses corrected Prop. 2.1 product, not Gotoda (3.3) B.',
    },
    numbers: {
      Pstar,
      PstarClosedForm: Pclosed,
      ...
      absDiffVsTarget: Math.abs(Pclosed - TARGET_DIGITS),
      minpolySexticResidual: minpolySextic(Pstar),
      minpolyCubicResidual: minpolyCubicQ(Pstar * Pstar),
    },
    checks: checks.filter((c) =>
      /closed_form|minpoly/.test(c.name)
    ),
    pass: checks.filter((c) => /closed_form|minpoly/.test(c.name)).every((c) => c.ok),
    sources: [
      'identities/sources/unequal-mu-half-draft-2026-09-23.md',
      'identities/sources/unequal-mu-half-Pstar-closed-2026-09-23.md',
    ],
  };
  fs.writeFileSync(pstarPath, JSON.stringify(pstarResult, null, 2) + '\n');

  console.log(JSON.stringify({ pass: allOk, outPath, pstarPath, Pstar, Pclosed, cosStar }, null, 2));
