...
//    The tab's own arctic-radius and free-area measurement is run over independent seeds on four
//    hexagons through the full regenerate() path; the across-seed mean and spread are compared with
//    the limit shape and with the per-plate error bars the status line prints. The free area is
//    recomputed from an independent breadth-first frozen test.
...
const HOOKS = ['  /* ---------- Lozenge Tilings ---------- */',
  '  Studio.auditHooks = { makeJob, runJob, buildTiling, corners, classify, measureArctic, ellipseOf, macmahonLog10, samplerSelfTest };\n  /* ---------- Lozenge Tilings ---------- */'];
const AUDIT_BODY = 'auditRead() { const st = host.getState(); if (!tile) return { building, tile: null }; const h = []; for (let n = 0; n < st.a * st.b; n++) h.push(tile.rz[n] / 2); ' +
  ...
  'meas: meas && { ring: meas.ring, rMean: meas.rMean, rSe: meas.rSe, tau: meas.tau, neff: meas.neff, disFrac: meas.disFrac, disSe: meas.disSe, predDisFrac: meas.predDisFrac, nTot: meas.nTot, nDis: meas.nDis, rs: Array.from(meas.rs), fA: Array.from(meas.fA), fB: Array.from(meas.fB) } }; },\n';
...
const SHAPES = [{ box: [24, 24, 24], seeds: 200 }, { box: [32, 32, 32], seeds: 150 }, { box: [48, 48, 48], seeds: 60 }, { box: [24, 40, 46], seeds: 60 }];
function limitShape(Lm) {
  const out = [], zSelf = [], chiSelf = [];
  let chunkCheck = null;
  for (const { box: [a, b, c], seeds } of SHAPES) {
    const t0 = Date.now(), state = S.stateFor(Lm.mod, { a, b, c, sampler: 'cftp', ring: 3, grain: 0 }, 'x'), inst = Lm.create(state), plates = [];
    let freeAreaChecked = 0;
    for (let i = 0; i < seeds; i++) {
      state.seed = 'lozenge-arctic-' + [a, b, c].join('x') + '-' + i;
      inst.run();
      const r = inst.inst.auditRead(), m = r.meas;
      assert(r.sound && r.info.exact && m, 'plate did not finish exact and sound');
      ...
      if (i < 2) {  // the free area from an independent breadth-first frozen test
        const reg = L.region(a, b, c), tl = L.tilingOfFaces(reg, L.faces(r.h, a, b, c)), orient = new Int8Array(reg.tris.length), F = L.faces(r.h, a, b, c);
        tl.faceTris.forEach((pair, f) => { for (const t of pair) orient[t] = F[f].o; });
        const fz = L.frozenBall(reg, orient, 3);
        let dis = 0, mismatch = 0;
        reg.tris.forEach((T, k) => { const idx = (T.u - r.U0) * r.VW + (T.v - r.V0); if ((T.t === 0 ? r.TA : r.TB)[idx] !== orient[k]) mismatch++; if ((T.t === 0 ? m.fA : m.fB)[idx] !== fz[k]) mismatch++; if (!fz[k]) dis++; });
        assert.equal(mismatch, 0, 'frozen flags or orientations differ from the independent reconstruction');
        assert.equal(dis / reg.tris.length, m.disFrac, 'free area differs from the independent frozen test');
        freeAreaChecked++;
      }
      if (!chunkCheck && a === 24) {
        const again = Lm.create(Object.assign({}, state)); again.run(40);
        const r2 = again.inst.auditRead();
        assert.deepEqual(r2.h, r.h); assert.deepEqual(r2.info, r.info); assert.equal(r2.meas.rMean, m.rMean);
      ...
      plates.push({ rMean: m.rMean, rSe: m.rSe, tau: m.tau, neff: m.neff, disFrac: m.disFrac, disSe: m.disSe, pred: m.predDisFrac, T: r.info.T, coal: r.info.coal, selfZ: r.self.z });
    }
    ...
    const calib = (vals, ses) => {
      ...
      return { sdAcrossSeeds: sdAcross, rmsPerPlateSe: rmsSe, ratio, ratio95: [lo, hi], sdOfPerPlateZ: S.stats.sd(z),
    ...
    const row = { box: [a, b, c], seeds, seedPattern: "'lozenge-arctic-" + [a, b, c].join('x') + "-<i>'", ring: 3,
      ...
      tau: S.stats.ensemble(plates.map(p => p.tau)), cftpStartBack: S.stats.ensemble(plates.map(p => p.T)), meetingSweep: S.stats.ensemble(plates.map(p => p.coal)),
      allExact: true, freeAreaIndependentlyChecked: freeAreaChecked, ms: Date.now() - t0 };
    out.push(row);
    log('limit ' + [a, b, c].join('x') + ' seeds ' + seeds + ': radius ' + rm.mean.toFixed(4) + ' +/- ' + rm.se.toFixed(4) + ' (' + row.radius.sigma.toFixed(1) + ' sigma), calib ' + row.radius.calibration.ratio.toFixed(2) +
      '; free ' + fm.mean.toFixed(4) + ' +/- ' + fm.se.toFixed(4) + ' vs ' + pred.toFixed(4) + ' (' + row.freeArea.sigma.toFixed(1) + ' sigma), calib ' + row.freeArea.calibration.ratio.toFixed(2) + ' (' + (Date.now() - t0) + ' ms)');
  }
  const z = S.stats.ensemble(zSelf), totalChi = chiSelf.reduce((s, v) => s + v, 0), df = 19 * chiSelf.length;
  ...
  return { shapes: out, selfTest, chunkCheck };
}
...
const FIXTURES = [
  ...
  { a: 36, b: 36, c: 36, aspect: '1:1', fill: 'frozen', shift: 2, inset: 0.02, strokeWidth: 0, arctic: 'both', curveW: 2, margin: 0.06, palette: 'graphite' },
...
async function browser() {
  ...
  try {
    ...
    for (let f = 0; f < FIXTURES.length; f++) {
      ...
      const params = Object.assign({ sampler: 'cftp', ring: 3, grain: 0 }, fx, { palette: pal.colors, bg: pal.bg });
      await page.evaluate(({ seed, params }) => { location.hash = 'lozenge/' + seed + '/' + btoa(JSON.stringify(Object.assign({}, params, { v: 2 }))); }, { seed, params });
      ...
      const fz = L.frozenBall(reg, orient, s.ring);
      let mismatch = 0;
      ...
      const wrong = checkSvg(module.svg, sceneReference(s, other, L.frozenBall(reg, oo, s.ring), reg, a.meas.rs, W, H), s, W, H);
      ...
      cases.push({ seed, parameters: s, box: [s.a, s.b, s.c], rhombi: scene.polys.length, heightsSha256: S.sha256(JSON.stringify(a.h)), cftp: a.info,
...
async function main() {
  ...
  const result = {
    ...
    scope: 'Coupling from the past as implemented in src/modules/lozenge.js (makeJob, runJob, sweep, buildTiling, corners, classify, measureArctic, ellipseOf, macmahonLog10, samplerSelfTest), run from the unmodified source. Exhaustive enumeration of tilings and plane partitions up to 4x4x4 with sampled frequencies on four boxes and the volume law on 4x4x4; the arctic radius and free area over independent seeds on four hexagons; seven complete rendered recipes with every SVG rhombus and curve point, every PNG byte and the shell vector RIP checked.',
    references: {
    ...
    controls, tabSelfTestUnmodified: { chi: selfUniform.chi, df: selfUniform.df, z: selfUniform.z, distinct: selfUniform.k },
