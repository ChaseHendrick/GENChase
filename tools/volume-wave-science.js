...
(async()=>{
  ...
  try{
    ...
    const result=await page.evaluate(async()=>{
      ...
      let reduced=true,budget={gpuSteps:1,gpuIntervalMs:0};
      const canvas=document.createElement('canvas');canvas.width=256;canvas.height=256;
      const instance=A.create({canvas,getState:()=>state,setStatus:s=>{status=s;},fault:s=>{throw Error(s);},isActive:()=>true,reducedMotion:()=>reduced,computeBudget:()=>budget});
      ...
      state={...state,running:true,warmup:0};instance.regenerate();await new Promise(resolve=>setTimeout(resolve,100));instance.action('cancel');const stopped=instance._auditRead();instance.resume();await new Promise(resolve=>setTimeout(resolve,100));const resumed=instance._auditRead();if(stopped.count!==resumed.count||error(stopped.values,resumed.values).max!==0)throw Error('Explicit stop was lost on resume');instance.pause();
      reduced=false;budget={gpuSteps:4,gpuIntervalMs:0};state={...state,grid:32,running:true,warmup:0};instance.regenerate();
      const batchStart=performance.now();while(instance._auditRead().count<4){if(performance.now()-batchStart>10000)throw Error('Maximum mode stalled');await new Promise(r=>setTimeout(r,10));}
      instance.pause();const batched=instance._auditRead();if(batched.count%4!==0)throw Error('Maximum batching not applied');
      reduced=true;budget={gpuSteps:1,gpuIntervalMs:0};state={...state,running:false,warmup:batched.count};instance.regenerate();await settle();instance.pause();
      if(error(batched.values,instance._auditRead().values).max!==0)throw Error('Batch scheduling changed the numerical field');
      return {maxBatchSteps:batched.count,batchReplayMaxDifference:0,scope:'Actual float32 atlas shader, periodic unit cube, zero initial velocity. Bounded discrete-mode, independent CPU, fixed-time continuum refinement and failure controls.',plates,stopSurvivesResume:true,modes,cpuComparison:{grid:n,steps,courant,error:cpuError},convergence,seedMaxError,controls:{wrongSignMaxDifference:wrongSign,stableNyquistMax:stableMax,aboveCflNyquistMax:unstableMax},sanitizerCflCases:cflCases,limitations:'No long-time, extreme 96..256 grid, cross-device, arbitrary parameter, experimental acoustics or broad print accuracy certification. Five selected preset print states preserve the field at 512x384; this does not establish physical calibration. Scientific GPU fixtures use grids 16..64 and at most 80 steps.'};
    });
