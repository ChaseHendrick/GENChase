...
(async()=>{
 ...
 const r=await page.evaluate(async()=>{
  ...
  const mod=def.create({canvas:c,util:Studio.util,getState:()=>s,setStatus:v=>{status=v;},isActive:()=>active,reducedMotion:()=>false});
  ...
  return {initialCount:initial.count,initialStep:initial.step,pauseStep:pause.step,pausedStable,exportStable,width:bitmap.width,height:bitmap.height,svgCircles:(svg.match(/<circle/g)||[]).length,ticks,maxGap,sliceMax:pause.sliceMax,replayEqual:JSON.stringify(fresh.x)===JSON.stringify(replay.x)&&JSON.stringify(fresh.y)===JSON.stringify(replay.y),replayStep:replay.step};
 });
 assert.equal(r.initialCount,16384);assert.equal(r.initialStep,0);assert(r.pausedStable&&r.exportStable&&r.replayEqual);assert.equal(r.width,1600);assert.equal(r.height,1000);assert.equal(r.svgCircles,16384);assert(r.ticks>10);assert(r.sliceMax<50);assert.equal(r.replayStep,8);
