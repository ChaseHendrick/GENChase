...
function hookSource(s) {
  ...
  s = replaceOnce(s, '      function status() { host.setStatus(',
    '      hooks.status = () => status();\n      function status() { (hooks.setStatus || host.setStatus)(');
  return s;
}
...
function main() {
  ...
  // 5. Total reflection: rays whose transmitted ray does not exist are skipped by the module (veselago.js:74).
  const tir = [];
  ...
  // 6. Grazing band reachable only through a URL hash: -sin(0.45) < n < -0.4 lets cos θ2 fall under the 0.05 slope clamp (veselago.js:83).
  ...
  const result = {
    ...
    limitations: [
      ...
      'Transmitted rays only. No Fresnel reflection is drawn at either face. A ray with no transmitted ray (|sin θ| > |n|) is skipped entirely at veselago.js:74, including its incident segment from the source; the plate and status line do not say so.',
      ...
      'The status label compares |Δx|/W with 0.12 (veselago.js:132). It is not a focus classifier: it reports "Veselago focus" for slabs with no real image behind them and cannot tell n = -1 from nearby n.',
      ...
      'Hash-only indices between -sin(0.45) and -0.4 reach the 0.05 slope clamp at veselago.js:83 for near-grazing rays; outside this domain.',
    ],
