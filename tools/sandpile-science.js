...
//      must be identical, and each must satisfy h_final = h_initial - Laplacian * odometer exactly.
...
//      (e + e)° = e and (c + e)° = c for random recurrent c. Failure control: (2m)° is not the identity.
//   4. Single source. The octant-folded pile equals a full-plane reference cell by cell for 2^12, 2^14 and
//      2^16 grains, and conserves every grain inside its truncation radius at all six menu counts.
...
//      with colors computed from the independent states, and exporting must leave the state unchanged.
...
const EDITS = {
  ...
  uneven: [
    [TOPPLE, 'const v = h[p]; if (v < 4) continue;\n        const t = v >> 2; let extra = v - 4 * t; h[p] = 0; pops++; if (hooks.odo) hooks.odo[p] += t;', 'uneven toppling'],
  ...
  threshold5: [[TOPPLE, 'const v = h[p]; if (v < 5) continue;\n        const t = (v / 5) | 0; h[p] = v - 5 * t; pops++;', 'threshold five, five removed']],
...
function abelian() {
  const plain = Hn.load(), counted = Hn.load({ edits: EDITS.odometer }), uneven = Hn.load({ edits: EDITS.uneven });
  const rows = [];
  let unevenDetected = 0, unevenConservationBroken = 0;
  const bySize = {}, byKind = {};
  for (const n of [8, 16, 32, 64]) for (let k = 0; k < 25; k++) {
    ...
    const tab = (loaded, how) => {
      ...
      return { h, odo, pops: top.pops(), chunks };
    };
    ...
    const topplings = o1.odo.reduce((a, b) => a + b, 0);
    ...
    const b = bySize[n] = bySize[n] || { configurations: 0, unevenDetected: 0, popsBelowTopplings: 0 };
    b.configurations++; if (differs) b.unevenDetected++; if (o1.pops < topplings) b.popsBelowTopplings++;
    const bk = byKind[kind] = byKind[kind] || { configurations: 0, unevenDetected: 0 }; bk.configurations++; if (differs) bk.unevenDetected++;
    rows.push({ seed, n, kind, grains: h0.reduce((a, v) => a + v, 0), topplings, popsLifo: o1.pops, popsShuffled: o2.pops, chunks: o3.chunks, finalSha256: Hn.sha256(Buffer.from(o1.h.buffer)), unevenControlDiffers: differs });
  }
  for (const n of Object.keys(bySize)) assert(bySize[n].unevenDetected > 0, 'uneven control not detected at n=' + n);
  log('PASS abelian: 100 configurations, 5 orders each, identical final states and odometers; h = h0 - Laplacian(odometer) exactly');
  ...
  return { configurations: rows.length, orders: ['tab LIFO (seedAll)', 'tab, shuffled push order', 'tab, interrupted every 4096 pops', 'independent FIFO, one toppling per visit', 'independent synchronous parallel'],
    identicalStates: true, identicalOdometers: true, conservationExact: true, bySize, byKind,
...
function tabIdentity(loaded, n) {
  ...
  while (!job.run(1e12));
  const r = S.read();
  return { e: Int32Array.from(r.heights), pops: r.topples };
}
function dhar() {
...
function identity() {
  ...
  for (const n of [64, 96, 128, 192, 256]) {
    ...
    const topplings = odoTab.reduce((a, b) => a + b, 0);
    rows.push({ grid: [n, n], identitySha256: Hn.sha256(Buffer.from(e.buffer)), heightCounts: hist, equalsIndependent: true, inLaplacianImage: true, recurrent: true, d4Symmetric: true, ePlusE: true,
      topplings, statusPops: plainTab.pops, recurrentTests: tests, seconds: (Date.now() - t0) / 1000 });
    log('PASS identity ' + n + 'x' + n + ': equals the independent e, in the Laplacian image, recurrent, symmetric, idempotent, (c + e) = c on ' + K + '; (2m)° control fails (' + ((Date.now() - t0) / 1000).toFixed(0) + ' s)');
  }
  return rows;
}
...
function singleSource() {
  ...
  for (const N of [4096, 16384, 65536, 131072, 262144, 1048576]) {
    const t0 = Date.now(), pile = plain.hooks.makeOctantPile(N);
    while (!pile.run(1e12));
    ...
    const row = { grains: N, truncationRadius: R, extent: ext, conserved: true, seconds: 0 };
    if (N <= 65536) {
      ...
      row.fullPlaneReference = { radius: Rr, extent: ref.extent, cellsCompared: cells, identical: true };
    }
    row.seconds = (Date.now() - t0) / 1000;
    rows.push(row);
    log('PASS single source ' + N + ': conserved inside R = ' + R + ' (extent ' + ext + ')' + (row.fullPlaneReference ? ', identical to the full-plane reference on ' + row.fullPlaneReference.cellsCompared + ' cells' : ''));
  }
  return rows;
}
...
function randomDrops() {
  ...
  for (const [n, drops, seed] of [[128, 30000, 'btw-1987'], [192, 50000, 'sandpile-soc-192']]) {
    ...
    const N = n * n, odo = new Float64Array(N); counted.hooks.odo = odo;
    const a = run(counted); counted.hooks.odo = null;
    const b = run(plain);
    assert(eqArr(a.heights, b.heights) && eqArr(a.marks, b.marks) && a.topples === b.topples, 'odometer side channel changed the run');
    ...
    const topplings = odo.reduce((x, y) => x + y, 0), refTopplings = ref.sizes.reduce((x, y) => x + y, 0);
    assert.equal(topplings, refTopplings); assert.equal(topplings, ref.batchTopplings);
    const maxRef = Math.max(...ref.sizes);
    rows.push({ grid: [n, n], drops, seed, heightsEqualBatchRelaxation: true, marksEqualReference: true, topplings, statusPops: b.topples, statusSumAv: b.sumAv, statusMaxAv: b.maxAv,
      referenceMaxAvalancheTopplings: maxRef, avalanches: ref.sizes.filter(s => s > 0).length });
    log('PASS random drops ' + n + 'x' + n + ', ' + drops + ' drops: heights = one batch relaxation, marks identical; topplings ' + topplings + ' (status counts ' + b.topples + ' pops)');
  }
  return rows;
}
  ...
  try {
    ...
    for (let k = 0; k < fixtures.length; k++) {
      ...
      if (st.mode === 'pile') {
        ...
        const n = st.size, h1 = new Float64Array(n * n).fill(6); stabilizeFIFO(h1, n, false);
        heights = h1.map(v => 6 - v); stabilizeFIFO(heights, n, false);
      ...
      cases.push({ name: fx.name, seed, parameters: st, state, heightsSha256: Hn.sha256(Buffer.from(Float64Array.from(heights).buffer)), stateMatchesIndependent: true, print, statePreserved: true });
      log('PASS print ' + fx.name + ': ' + print.pixels.toLocaleString() + ' pixels, max channel error ' + print.maxChannelError + ', ' + print.nearBoundaryPixels + ' near-boundary pixels (' + print.nearBoundaryShowingNeighborCell + ' show the neighbor cell); displaced control ' + print.displacedControlFailingPixels + ', wrong-state control ' + print.wrongStateControlFailingPixels[0]);
    }
