...
- An `rdxCreate` tab can take typed reaction terms by declaring `custom: { active(s), key(s), stepFS(s), boundKey(s), bound(s, samples), describe(s, info) }` (the Turing tab's Custom reaction is the one user). `stepFS` returns a step shader whose formulas come only from `U.expr.toGLSL`; the factory compiles it when `key` changes. Every 50 steps, and whenever `boundKey` changes, it samples the field (the center and the largest cell of each of 32 × 32 blocks, or a non-finite value if a block holds one), reads the samples back and asks `bound` for `{ ceiling, lamD, rho, span, halt }`; the step used is `min(dt, ceiling)`, and a `halt` stops the plate with the reason on the status line. The schedule depends only on the step count, so a recipe reprints whatever the chunking.
