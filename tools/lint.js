...
for (const [label, text] of [['studio.html', src], ['README.md', readme]]) {
  ...
  const claims = [...text.matchAll(/\b((?:One hundred (?:eight|nine|ten|eleven|twelve|thirteen))|(?:Forty|Fifty|Sixty|Seventy)(?:[- ](?:one|two|three|four|five|six|seven|eight|nine))?)\b(?=(?:\s+\w+){0,2}\s+(?:pattern-forming systems|sciences|techniques|tabs)\b)/gi)]
    .map(x => x[1]);
