...
| `Studio.getProvenance(id?)` | A detached record of what made the current plate: software, API and recipe versions, build, source file and its SHA-256, validation status (`technique.validation`, lowered by the instance's optional `evidence()` in a mode the tab's record does not cover, with the tab's own status in `tabValidation` and the reason in `validationNote`), recipe and link, the witness, and the device (WebGL2 renderer and vendor, or CPU; which render-target precisions the state used). Nothing time-dependent, so it is deterministic for a given recipe, build and device. |
