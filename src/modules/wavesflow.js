...
`;
  ...
  Studio.register({
    ...
    schema: GRID.concat([
      ...
      RANGE('Potential', 'V0', 'Strength V₀', GEOM, 0, 6, 0.02, f2, { dimUnless: usesV0,
        hint: 'Barrier height, well depth at the frame edge, lattice bump height, or disorder width. The packet energy is k²/2.' }),
      ...
      { group: 'Potential', key: 'wallMode', label: 'Wall', type: 'seg', kind: GEOM, options: [['hard', 'Hard'], ['absorbing', 'Absorbing']], dimUnless: usesSlit,
        hint: 'A hard wall reflects the packet back over the source, an absorbing one only lets the slits through.' },
      ...
      RANGE('Packet', 'k', 'Momentum k', GEOM, 0, 1.6, 0.02, f2, { hint: 'In 1/cell. Wavelength 2π/k; speed k cells per unit time. Above about 1.5 the grid cannot carry the wave.' }),
    ...
    closedGroups: ['Packet'],
    hints: {
      Potential: 'The slits, barrier and well are what the packet meets. V₀ against k²/2 decides whether it passes, tunnels, or is turned back.',
      ...
      Simulation: 'The time step is clamped against the Visscher bound dt < 2/(4 + V₀) so the leapfrog stays unitary. The absorber is an imaginary potential over the frame edge.',
      Picture: 'Detector is the print: |ψ|² summed over the run and shown with a film-like response. Density and Phase are the live wave.',
    },
    ...
    sanitize(s) {
      ...
      // Visscher: |E| dt < 2 with E up to 4/dx^2 + V0 on the 5-point Laplacian (dx = 1); stay well inside it
      const dtMax = Math.min(0.25, 1.6 / (4 + Math.abs(s.V0)));
  ...
  // State texture: r = Re psi (integer steps), g = Im psi (half steps), b = time-integrated |psi|^2 (the detector).
...
void main(){
  ...
  if (u_phase == 0) {
    ...
    outColor = vec4(R, c.g, c.b, 1.0);
  } else {
    // Im(t + 3dt/2) = Im(t + dt/2) - dt * H Re(t + dt); density at t + dt is Re^2 + Im_old * Im_new (Visscher)
    float HR = -0.5 * lapc(v_uv, px, 0) + V * c.r;
    float I = (c.g - u_dt * HR) * keep * damp;
    float dens = max(c.r * c.r + c.g * I, 0.0);
    outColor = vec4(c.r, I, c.b + u_dtE * dens, 1.0);
  }
}`;
...
void main(){
  ...
  float ph = dot(u_kv, d);
  outColor = vec4(c.r + u_amp * g * cos(ph), c.g + u_amp * g * sin(ph), c.b, 1.0);
}`;
...
void main(){
  ...
  if (u_view == 0) {
    ...
    float ph = atan(c.g, c.r) / 6.28318530718 + 0.5;
  ...
  // Per block: rg = 16-bit mean density (for the norm), b = log mean exposure, a = log max density.
...
void main(){
  ...
  function schrodingerCreate(host) {
    ...
    let nOff = 0, norm0 = 0, normNow = 0, expWhite = 0, densWhite = 0, simTime = 0;
    ...
    function packet(s) {
      ...
      for (let y = 0; y < gh; y++) for (let x = 0; x < gw; x++) {
        ...
        const i = (y * gw + x) * 4;
        data[i] = g * Math.cos(ph); data[i + 1] = g * Math.sin(ph); data[i + 2] = 0; data[i + 3] = 1;
        norm0 += g * g;
      }
      return data;
    }
    ...
    function status(extra) {
      ...
      host.setStatus(
        ...
        (s.view === 'density' || s.view === 'phase'
          ? '<span>norm <b>' + Math.round(100 * normNow / Math.max(norm0, 1e-9)) + '%</b> · t <b>' + simTime.toFixed(0) + '</b></span>'
    ...
    return {
      ...
      regenerate() {
        ...
        // Im lives on half steps: move it to t = dt/2 before the leapfrog starts
        halfStep(s, 0.5 * s.dt, 0, 1);
        normNow = norm0;
      ...
      disturb(p) {
        ...
        P.kick.draw(S.write, { u_p: S.read, u_res: [gw, gh], u_pos: [p.x, p.yGL], u_kv: kv, u_rad: Math.max(3, s.sigma * 0.6), u_amp: 0.5 });
        S.swap();
        render();
        if (!rig.raf) loop();
      },
  ...
  Studio.register({
    ...
    blurb: 'A pan of fluid, hot floor and cold lid. Below Ra ≈ 1708 heat simply conducts and nothing moves. Above it the layer rolls over in cells whose width is about the depth of the pan. Push Ra to a hundred thousand and the rolls give way to plumes: thin thermal boundary layers grow on the plates until a blob of hot fluid tears free and rises as a mushroom, its cold twin falling from the lid. Push further and the plumes fight each other in a turbulent soup. The plate shows temperature, or the vorticity that drives it, or a schlieren view of the temperature gradient, the way a shadowgraph shows convection in a lab. Drag to drop a warm blob into the flow.',
    schema: GRID.concat([
      RANGE('Convection', 'logRa', 'Rayleigh number', LIVE, 3, 7, 0.05, fmtRa, {
        hint: 'Buoyancy against viscosity and diffusion. Onset at 1708 (rigid plates). 10⁴ to 10⁵: steady cells. 10⁵ to 10⁶: plumes. Higher: turbulent (under-resolved at these grids).' }),
    ...
    hints: {
      Convection: 'Ra is the whole story: conduction, cells, plumes, turbulence, in that order. Pr decides whether the plumes are wispy (low) or blobby (high). Time is in free-fall units, so a plume crosses the layer in about one.',
    ...
    sanitize(s) {
      ...
      s.dt = U.clamp(Math.min(Number(s.dt) || 0.008, convDtMax(s)), 0.00005, 0.03);
    },
    surprise(rng) {
...
void main(){
  ...
  if (wall) { v = 0.0; if (u_wall == 0) u = 0.0; }
...
void main(){
  ...
  // Per block: rg = 16-bit (0.5 + mean v T) for the Nusselt estimate, b = log max |omega|, a = log max |grad T|.
...
void main(){
  ...
  float m = clamp(0.5 + svt / 64.0, 0.0, 1.0) * 255.0;
  float hi = floor(m) / 255.0, lo = fract(m);
  ...
  function convectionCreate(host) {
    ...
    function status(extra) {
      const s = host.getState();
      const Ra = raOf(s);
      const regime = Ra < 1708 ? 'conductive' : Ra < 2e4 ? 'rolls' : Ra < 5e5 ? 'plumes' : 'turbulent';
      host.setStatus(
        '<span>grid <b>' + gw + '×' + gh + '</b></span>' +
        '<span>Ra <b>' + fmtRa(s.logRa) + '</b> · Pr <b>' + s.Pr.toFixed(1) + '</b> · ' + regime + '</span>' +
        '<span>Nu ≈ <b>' + Math.max(1, nusselt).toFixed(1) + '</b> · t <b>' + simTime.toFixed(1) + '</b></span>' +
        '<span>step <b>' + rig.stepCount.toLocaleString() + '</b></span>' +
        (extra ? '<span>' + extra + '</span>' : '')
      );
    }
