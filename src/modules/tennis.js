...
(function () {
  ...
  const PRESETS = {
    ...
    stable1: pre('Stable I1', { I2: 1.05, eps: 0.01, T: 50 }, Pal.harbor),
  ...
  Studio.register({
    ...
    create(host) {
      ...
      function compute() {
        ...
        for (let y = 0; y < H; y++) {
          for (let k = 0; k < 8; k++) {
            const a1 = ((I2 - I3) / I1) * w2 * w3;
            const a2 = ((I3 - I1) / I2) * w3 * w1;
            const a3 = ((I1 - I2) / I3) * w1 * w2;
            w1 += dt * a1; w2 += dt * a2; w3 += dt * a3;
          }
          if (prev * w2 < 0) flips++;
          prev = w2;
          for (let x = 0; x < W; x++) {
      ...
      function status() { host.setStatus('<span>I2 <b>' + f2(extra) + '</b> in (1, 2)</span><span>ω2 flips <b>' + (metric | 0) + '</b></span><span>' + (metric > 0 ? 'unstable axis' : 'stable') + '</span>'); }
