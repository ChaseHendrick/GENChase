...
`;
  ...
  function create(host){
    ...
    let field=null,n=0,shape=null,dt=0,count=0,pending=0,raf=0,fence=null,ramp=null,rampKey='',failed=false;
    ...
    function status(){const bytes=shape.w*shape.h*32;host.setStatus('<span>grid <b>'+n+'³</b> · float32</span><span>textures <b>'+(bytes/1048576).toFixed(1)+' MiB</b></span><span>step <b>'+count+'</b> · t '+(count*dt).toFixed(3)+'</span><span>dt '+dt.toExponential(2)+(pending?' · '+pending+' queued':'')+'</span>');}
    ...
    function schedule(){
      if(raf||failed||!field||!host.isActive())return;
    ...
    return {
      aspect:()=>1,
      regenerate(){stop();pending=0;failed=false;if(fence){gl.deleteSync(fence);fence=null;}const s=host.getState(),next=layout(s.grid),max=gl.getParameter(gl.MAX_TEXTURE_SIZE);
      ...
      resize(){if(field)render();},repaint(){if(field)render();},pause:stop,resume:schedule,live(){stop();if(!host.getState().running)pending=0;schedule();},
      action(key){if(key==='cancel'){stop();pending=0;status();}else if(key==='burst'&&field){pending+=128;schedule();}else if(key==='reseed')this.regenerate();},
