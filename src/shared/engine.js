...
void main(){
  ...
  async function startStudio(options) {
    ...
    $('tabs').addEventListener('keydown', async ev => {
      ...
      // Only the tabs a search leaves visible; moving onto a hidden tab switches to a plate nobody can see.
      const list = [...document.querySelectorAll('.tab[data-id]:not(.is-hidden)')];
      if (!list.length) return;
    ...
    document.addEventListener('keydown', ev => {
      ...
      // Ctrl, Cmd and Alt chords belong to the browser (find, print, reload, save); only undo is ours.
      if ((ev.metaKey || ev.ctrlKey || ev.altKey) && ev.key !== 'z' && ev.key !== 'Z') return;
